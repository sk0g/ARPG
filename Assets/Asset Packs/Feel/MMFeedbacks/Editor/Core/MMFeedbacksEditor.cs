...
public class MMFeedbacksEditor : Editor
{
    ...
    static class FeedbackCopy
    {
        ...
        static public void Paste(SerializedObject target)
        {
            ...
            {
                for (int i = 0; i < Properties.Count; i++) { target.CopyFromSerializedProperty(Properties[i]); }
            }
        }
    ...
    void OnEnable()
    {
        ...
        for (int i = 0; i < _typesAndNames.Count; i++) { typeNames.Add(_typesAndNames[i].FeedbackName); }
    ...
    public override void OnInspectorGUI()
    {
        ...
        Rect durationRect =
            new Rect(helpBoxRect.xMax - durationRectWidth, helpBoxRect.yMax + 6, durationRectWidth, 17f);
        ...
        Rect directionRect = new Rect(helpBoxRect.xMax - directionRectWidth, helpBoxRect.yMax + 5, directionRectWidth,
                                      17f);
        ...
        for (int i = 0; i < _mmfeedbacks.arraySize; i++)
        {
            ...
            {
                if ((feedback as MMFeedbackLooper).InfiniteLoop) { label = label + "[Infinite Loop] "; }
                else { label = label + "[ " + (feedback as MMFeedbackLooper).NumberOfLoopsLeft + " loops left ] "; }
            }
        ...
        EditorGUILayout.BeginHorizontal();
        {
            ...
            if (FeedbackCopy.HasCopy())
            {
                if (GUILayout.Button("Paste as new", EditorStyles.miniButton,
                                     GUILayout.Width(EditorStyles.miniButton.CalcSize(new GUIContent("Paste as new"))
                                                                 .x))) { PasteAsNew(); }
            }
            ...
            if (FeedbackCopy.HasMultipleCopies())
            {
                if (GUILayout.Button("Paste all as new", EditorStyles.miniButton,
                                     GUILayout.Width(EditorStyles.miniButton
                                                                 .CalcSize(new GUIContent("Paste all as new"))
                                                                 .x))) { PasteAllAsNew(); }
            }
        }
        ...
        if (wasRemoved)
        {
            ...
            {
                if (c != null) { c.hideFlags = HideFlags.None; }
            }
        }
        ...
        EditorGUILayout.BeginHorizontal();
        {
            ...
            {
                if (GUILayout.Button("Pause", EditorStyles.miniButtonMid)) { (target as MMFeedbacks).PauseFeedbacks(); }
            }
