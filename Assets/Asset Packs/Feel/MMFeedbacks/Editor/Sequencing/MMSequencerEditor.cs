...
public class MMSequencerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        _boxesPerLine = (int) Mathf.Round(
                            (_inspectorWidth
                             - ((_targetSequencer.Sequence.SequenceTracks.Count) * _distanceBetweenButtons)
                             - _trackControlWidth
        ...
        EditorGUILayout.Space();
        DrawSequenceIndexes();
        for (int i = 0; i < _targetSequencer.Sequence.SequenceTracks.Count; i++) { DrawTrack(i); }
    ...
    protected virtual void DrawControlButtons()
    {
        ...
        {
            if (GUILayout.Button("Stop Playing")) { _targetSequencer.StopSequence(); }
        }
        else
        {
            if (GUILayout.Button("Start Playing")) { _targetSequencer.PlaySequence(); }
        }

        if (GUILayout.Button("Play Next Beat")) { _targetSequencer.PlayBeat(); }
    }
    ...
    protected virtual void DrawTrack(int trackIndex)
    {
        ...
        if (_targetSequencer.Sequence.QuantizedSequence == null) { return; }
        ...
        else { _trackControlColor = _emptyButtonColor; }
        ...
        for (int i = 0; i < _targetSequencer.Sequence.QuantizedSequence[trackIndex].Line.Count; i++)
        {
            ...
            {
                if (i % 4 == 0) { _buttonColor = _empty4ButtonColor; }
                else { _buttonColor = _emptyButtonColor; }
            }

            // if the track is inactive, we reduce colors
            if (!_targetSequencer.Sequence.SequenceTracks[trackIndex].Active) { _buttonColor = _buttonColor / 2f; }
    ...
    protected virtual void DrawSequenceButton(int trackIndex, int sequenceIndex, Color buttonColor)
    {
        ...
        {
            if (_targetSequencer.BeatThisFrame) { _buttonColor = Color.white; }
            else { _buttonColor = SequencerColor(_targetSequencer.LastBeatTimestamp, buttonColor); }
        }
