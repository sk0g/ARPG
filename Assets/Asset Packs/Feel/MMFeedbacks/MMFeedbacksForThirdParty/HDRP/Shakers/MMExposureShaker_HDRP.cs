...
public class MMExposureShaker_HDRP : MMShaker
{
    ...
    {
        float newValue = ShakeFloat(ShakeFixedExposure, RemapFixedExposureZero, RemapFixedExposureOne,
                                    RelativeIntensity, _initialFixedExposure);
    ...
    public virtual void OnExposureShakeEvent(AnimationCurve intensity, float duration, float remapMin, float remapMax,
...
public struct MMExposureShakeEvent_HDRP
{
    ...
    static public void Trigger(AnimationCurve fixedExposure, float duration, float remapMin, float remapMax,
        bool relativeFixedExposure = false,
        float attenuation = 1.0f, int channel = 0, bool resetShakerValuesAfterShake = true,
        bool resetTargetValuesAfterShake = true, bool forwardDirection = true,
        TimescaleModes timescaleMode = TimescaleModes.Scaled, bool stop = false)
    {
        OnEvent?.Invoke(fixedExposure, duration, remapMin, remapMax, relativeFixedExposure, attenuation, channel,
                        resetShakerValuesAfterShake, resetTargetValuesAfterShake, forwardDirection, timescaleMode,
                        stop);
    }
}
}