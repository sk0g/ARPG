...
public class MMCinemachineCameraShaker : MonoBehaviour
{
    ...
    [Tooltip("the amplitude of the camera's noise when it's idle")] [MMFReadOnly]
    public float IdleAmplitude;

    /// the frequency of the camera's noise when it's idle
    [Tooltip("the frequency of the camera's noise when it's idle")] [MMFReadOnly]
    ...
    [MMFInspectorButton("TestShake")] public bool TestShakeButton;
