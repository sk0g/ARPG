...
public abstract class MMFeedbackBase : MMFeedback
{
    ...
    [Tooltip("how long the target property should change over time")] [MMFEnumCondition("Mode", (int) Modes.OverTime)]
    ...
    protected virtual void Instant()
    {
        ...
        foreach (MMFeedbackBaseTarget target in _targets) { target.Target.SetLevel(target.InstantLevel); }
    }
    ...
    protected virtual void SetValues(float time, float feedbacksIntensity)
    {
        ...
        {
            float intensity =
                MMTween.Tween(time, 0f, 1f, target.RemapLevelZero, target.RemapLevelOne, target.LevelCurve);
            if (RelativeValues) { intensity += target.InitialLevel; }
