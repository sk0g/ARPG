...
public class MMFeedbackFloatingText : MMFeedback
{
    ...
    [Tooltip(
        "the channel to require a floating text spawn on. This has to match the Channel value in the general settings of your chosen MMFloatingTextSpawner")]
    ...
    [Tooltip("the gradient to apply over the lifetime of the text")] [GradientUsage(true)]
    ...
    [Tooltip("the forced lifetime for the spawned text")] [MMFCondition("ForceLifetime", true)]
    ...
    {
        get { return ApplyTimeMultiplier(Lifetime); }
        set { Lifetime = value; }
    }
    ...
    protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        if (Active)
        {
            ...
            MMFloatingTextSpawnEvent.Trigger(Channel, _playPosition, _value, Direction, Intensity * intensityMultiplier,
                                             ForceLifetime, Lifetime, ForceColor, AnimateColorGradient,
                                             Timing.TimescaleMode == TimescaleModes.Unscaled);
        }
    }
}
}