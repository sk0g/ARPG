...
public class MMInputSequenceRecorder : MonoBehaviour
{
    ...
    public virtual void Initialization()
    {
        ...
        if (SequenceScriptableObject == null)
        {
            Debug.LogError(this.name
                           + " this input based sequencer needs a bound scriptable object to function, please create one and bind it in the inspector.");
        }
    }
    ...
    {
        if (StartRecordingOnGameStart) { StartRecording(); }
    }
    ...
    {
        Recording = true;
        if (!AdditiveRecording) { SequenceScriptableObject.OriginalSequence.Line.Clear(); }
    ...
    protected virtual void DetectStartAndEnd()
    {
        if (!Recording)
        {
            if (Input.GetKeyDown(StartRecordingHotkey)) { StartRecording(); }
        }
        else
        {
            if (Input.GetKeyDown(StartRecordingHotkey)) { StopRecording(); }
        }
    }
    ...
    protected virtual void DetectRecording()
    {
        if (Recording && (SequenceScriptableObject != null))
        {
            ...
            {
                if (Input.GetKeyDown(track.Key)) { AddNoteToTrack(track); }
            }
        }
    }
