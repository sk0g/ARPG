...
public class MMSequence : ScriptableObject
{
    ...
    public virtual void QuantizeSequenceToBPM(List<MMSequenceNote> baseSequence)
    {
        ...
        for (int i = 0; i < numberOfBeatsInSequence; i++) { _quantizedBeats[i] = i * beatDuration; }
    ...
    {
        for (int i = 0; i < SequenceTracks.Count; i++) { SequenceTracks[i].SetDefaults(i); }
    }
    ...
    {
        foreach (MMSequenceTrack track in SequenceTracks) { track.TrackColor = RandomSequenceColor(); }
    }
    ...
    public static float RoundFloatToArray(float value, float[] array)
    {
        ...
        if (array[max] - value <= value - array[min]) { return array[max]; }
