...
public class MMAudioSourceSequencer : MMSequencer
{
    ...
    protected override void OnBeat()
    {
        ...
        for (int i = 0; i < Sequence.SequenceTracks.Count; i++)
        {
            ...
            {
                if ((AudioSources.Count > i) && (AudioSources[i] != null)) { AudioSources[i].Play(); }
            }
        }
    }
    ...
    public virtual void SetupSounds()
    {
        ...
        if (AudioSources.Count < Sequence.SequenceTracks.Count)
        {
            for (int i = 0; i < Sequence.SequenceTracks.Count; i++)
            {
                if (i >= AudioSources.Count) { AudioSources.Add(null); }
            }
        }
        ...
        {
            AudioSources.Clear();
            for (int i = 0; i < Sequence.SequenceTracks.Count; i++) { AudioSources.Add(null); }
        }
    }
}
}