...
public class MMSoundSequencer : MMSequencer
{
    ...
    protected override void OnBeat()
    {
        ...
        for (int i = 0; i < Sequence.SequenceTracks.Count; i++)
        {
            ...
            {
                if ((_audioSources.Count > i) && (_audioSources[i] != null)) { _audioSources[i].Play(); }
            }
        }
    }
    ...
    public virtual void SetupSounds()
    {
        ...
        if (Sounds.Count < Sequence.SequenceTracks.Count)
        {
            for (int i = 0; i < Sequence.SequenceTracks.Count; i++)
            {
                if (i >= Sounds.Count) { Sounds.Add(null); }
            }
        }
        ...
        {
            Sounds.Clear();
            for (int i = 0; i < Sequence.SequenceTracks.Count; i++) { Sounds.Add(null); }
        }
    }
}
}