...
public class MMSequencer : MonoBehaviour
{
    ...
    [HideInInspector] public float LastBeatTimestamp = 0f;
    ...
    public virtual void ClearSequence()
    {
        ...
        {
            for (int i = 0; i < SequencerLength; i++) { Sequence.QuantizedSequence[trackIndex].Line[i].ID = -1; }
        }
    }
    ...
    public virtual void PlayBeat()
    {
        ...
        for (int i = 0; i < Sequence.SequenceTracks.Count; i++)
        {
            ...
            {
                if (TrackEvents[i] != null) { TrackEvents[i].Invoke(); }
            }
        }
        ...
        if (RandomSequence) { CurrentSequenceIndex = UnityEngine.Random.Range(0, SequencerLength); }
    }
    ...
    public virtual void ToggleStep(int stepIndex)
    {
        ...
        for (int trackIndex = 0; trackIndex < Sequence.SequenceTracks.Count; trackIndex++)
        {
            if (active) { Sequence.QuantizedSequence[trackIndex].Line[stepIndex].ID = -1; }
            else { Sequence.QuantizedSequence[trackIndex].Line[stepIndex].ID = Sequence.SequenceTracks[trackIndex].ID; }
        }
    }
    ...
    public virtual void SetupTrackEvents()
    {
        ...
        if (TrackEvents.Count < Sequence.SequenceTracks.Count)
        {
            for (int i = 0; i < Sequence.SequenceTracks.Count; i++)
            {
                if (i >= TrackEvents.Count) { TrackEvents.Add(new UnityEvent()); }
            }
        }
        ...
        {
            TrackEvents.Clear();
            for (int i = 0; i < Sequence.SequenceTracks.Count; i++) { TrackEvents.Add(new UnityEvent()); }
        }
    }
}
}