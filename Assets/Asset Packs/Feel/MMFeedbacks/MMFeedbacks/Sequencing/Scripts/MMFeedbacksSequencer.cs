...
public class MMFeedbacksSequencer : MMSequencer
{
    ...
    protected override void OnBeat()
    {
        ...
        for (int i = 0; i < Sequence.SequenceTracks.Count; i++)
        {
            ...
            {
                if ((Feedbacks.Count > i) && (Feedbacks[i] != null)) { Feedbacks[i].PlayFeedbacks(); }
            }
        }
    }
    ...
    public virtual void SetupFeedbacks()
    {
        ...
        if (Feedbacks.Count < Sequence.SequenceTracks.Count)
        {
            for (int i = 0; i < Sequence.SequenceTracks.Count; i++)
            {
                if (i >= Feedbacks.Count) { Feedbacks.Add(null); }
            }
        }
        ...
        {
            Feedbacks.Clear();
            for (int i = 0; i < Sequence.SequenceTracks.Count; i++) { Feedbacks.Add(null); }
        }
    }
}
}