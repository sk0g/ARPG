...
public class MMFeedbackMaterial : MMFeedback
{
    ...
    {
        get { return (InterpolateTransition) ? TransitionDuration : 0f; }
        set
        {
            if (InterpolateTransition) { TransitionDuration = value; }
        }
    }
    ...
    /// the selected method
    [Tooltip("the selected method")] public Methods Method;

    /// whether or not the sequential order should loop
    [MMFEnumCondition("Method", (int) Methods.Sequential)] [Tooltip("whether or not the sequential order should loop")]
    ...
    {
        return (Timing.TimescaleMode == TimescaleModes.Scaled) ? Time.deltaTime : Time.unscaledDeltaTime;
    }
    ...
    protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        ...
        else { TargetRenderer.material = Materials[newIndex]; }
    }
    ...
    protected virtual int DetermineNextIndex()
    {
        ...
        case Methods.Random:
            ...
            {
                while (_currentIndex == random) { random = Random.Range(0, Materials.Count); }
            }
        ...
        case Methods.Sequential:
            _currentIndex++;
            if (_currentIndex >= Materials.Count) { _currentIndex = Loop ? 0 : _currentIndex; }
