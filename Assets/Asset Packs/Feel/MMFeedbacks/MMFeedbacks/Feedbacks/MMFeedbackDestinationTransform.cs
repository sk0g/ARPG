...
public class MMFeedbackDestinationTransform : MMFeedback
{
    ...
    [Tooltip("the transform to use as origin in ForceOrigin mode")] [MMFCondition("ForceOrigin", true)]
    ...
    [Tooltip("the curve to use to animate the position on")] [MMFCondition("SeparatePositionCurve", true)]
    ...
    [Tooltip("the curve to use to animate the rotation on")] [MMFCondition("SeparateRotationCurve", true)]
    ...
    [Tooltip("the curve to use to animate the scale on")] [MMFCondition("SeparateScaleCurve", true)]
    ...
    {
        if (Active && (TargetTransform != null)) { _coroutine = StartCoroutine(AnimateToDestination()); }
    }
    ...
    protected virtual IEnumerator AnimateToDestination()
    {
        ...
        while ((journey >= 0) && (journey <= Duration) && (Duration > 0))
        {
            ...
            _newRotation =
                Quaternion.LerpUnclamped(_pointARotation, _pointBRotation, _animationCurve.Evaluate(percent));
    ...
    protected override void CustomStopFeedback(Vector3 position, float feedbacksIntensity = 1)
    {
        ...
        if (Active && (TargetTransform != null) && (_coroutine != null)) { StopCoroutine(_coroutine); }
    }
}
}