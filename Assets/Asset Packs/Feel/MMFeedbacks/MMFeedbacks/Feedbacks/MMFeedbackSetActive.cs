...
[AddComponentMenu("")]
[FeedbackHelp(
    "This feedback allows you to change the state of the target gameobject from active to inactive (or the opposite), on init, play, stop or reset. For each of these you can specify if you want to force a state (active or inactive), or toggle it (active becomes inactive, inactive becomes active).")]
...
public class MMFeedbackSetActive : MMFeedback
{
    ...
    [Tooltip("how to change the state on play")] [MMFCondition("SetStateOnPlay", true)]
    ...
    protected override void CustomInitialization(GameObject owner)
    {
        ...
        {
            if (SetStateOnInit) { SetStatus(StateOnInit); }
        }
    }
