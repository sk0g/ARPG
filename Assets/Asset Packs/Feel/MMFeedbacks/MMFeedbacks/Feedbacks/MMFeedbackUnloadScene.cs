...
public class MMFeedbackUnloadScene : MMFeedback
{
    ...
    {
        get { return MMFeedbacksInspectorColors.SceneColor; }
    }
#endif
    ...
    [Tooltip("the name of the scene to unload")] [MMFEnumCondition("Method", (int) Methods.SceneName)]
    ...
    protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        ...
        if (Method == Methods.BuildIndex) { _sceneToUnload = SceneManager.GetSceneByBuildIndex(BuildIndex); }
        else { _sceneToUnload = SceneManager.GetSceneByName(SceneName); }

        if ((_sceneToUnload != null) && (_sceneToUnload.isLoaded)) { SceneManager.UnloadSceneAsync(_sceneToUnload); }
        else
        {
