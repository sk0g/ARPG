...
[FeedbackHelp(
    "This feedback will animate the target's scale on the 3 specified animation curves, for the specified duration (in seconds). You can apply a multiplier, that will multiply each animation curve value.")]
public class MMFeedbackScale : MMFeedback
{
    ...
    [Tooltip("the object to animate")] public Transform AnimateScaleTarget;
    ...
    [Tooltip("the value to remap the curve's 1 value to")] [FormerlySerializedAs("Multiplier")]
    ...
    [Tooltip("the x scale animation definition")] [MMFCondition("AnimateX", true)]
    public AnimationCurve AnimateScaleX =
        new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1.5f), new Keyframe(1, 0));
    ...
    [Tooltip("the y scale animation definition")] [MMFCondition("AnimateY", true)]
    ...
    [Tooltip("the z scale animation definition")] [MMFCondition("AnimateZ", true)]
    ...
    {
        get { return ApplyTimeMultiplier(AnimateScaleDuration); }
        set { AnimateScaleDuration = value; }
    }
    ...
    protected override void CustomInitialization(GameObject owner)
    {
        ...
        if (Active && (AnimateScaleTarget != null)) { GetInitialScale(); }
    }
    ...
    protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        ...
        {
            if (DetermineScaleOnPlay && NormalPlayDirection) { GetInitialScale(); }
            ...
            if (isActiveAndEnabled || _hostMMFeedbacks.AutoPlayOnEnable)
            {
                if ((Mode == Modes.Absolute) || (Mode == Modes.Additive))
                {
                    ...
                    _coroutine = StartCoroutine(AnimateScale(AnimateScaleTarget, Vector3.zero, FeedbackDuration,
                                                             AnimateScaleX, AnimateScaleY, AnimateScaleZ,
                                                             RemapCurveZero * intensityMultiplier,
    ...
    {
        if (AnimateScaleTarget == null) { yield break; }

        if ((AnimateScaleX == null) || (AnimateScaleY == null) || (AnimateScaleZ == null)) { yield break; }

        if (FeedbackDuration == 0f) { yield break; }
        ...
        while ((journey >= 0) && (journey <= FeedbackDuration) && (FeedbackDuration > 0))
        {
            ...
            if (AnimateX)
            {
                _newScale.x = Mathf.LerpUnclamped(_initialScale.x, DestinationScale.x,
                                                  AnimateScaleX.Evaluate(percent) + Offset);
                _newScale.x = MMFeedbacksHelpers.Remap(_newScale.x, 0f, 1f, RemapCurveZero, RemapCurveOne);
            }

            if (AnimateY)
            {
                _newScale.y = Mathf.LerpUnclamped(_initialScale.y, DestinationScale.y,
                                                  AnimateScaleY.Evaluate(percent) + Offset);
                _newScale.y = MMFeedbacksHelpers.Remap(_newScale.y, 0f, 1f, RemapCurveZero, RemapCurveOne);
            }

            if (AnimateZ)
            {
                _newScale.z = Mathf.LerpUnclamped(_initialScale.z, DestinationScale.z,
                                                  AnimateScaleZ.Evaluate(percent) + Offset);
            ...
            else { journey += NormalPlayDirection ? Time.unscaledDeltaTime : -Time.unscaledDeltaTime; }
    ...
    /// <returns></returns>
    protected virtual IEnumerator AnimateScale(Transform targetTransform, Vector3 vector, float duration,
        AnimationCurve curveX, AnimationCurve curveY, AnimationCurve curveZ, float remapCurveZero = 0f,
        float remapCurveOne = 1f)
    {
        ...
        while ((journey >= 0) && (journey <= duration) && (duration > 0))
        {
            ...
            if (AnimateX)
            {
                ...
                if (Mode == Modes.Additive) { vector.x += _initialScale.x; }
            }
            else { vector.x = targetTransform.localScale.x; }
            ...
            if (AnimateY)
            {
                ...
                if (Mode == Modes.Additive) { vector.y += _initialScale.y; }
            }
            else { vector.y = targetTransform.localScale.y; }
            ...
            if (AnimateZ)
            {
                ...
                if (Mode == Modes.Additive) { vector.z += _initialScale.z; }
            }
            else { vector.z = targetTransform.localScale.z; }
