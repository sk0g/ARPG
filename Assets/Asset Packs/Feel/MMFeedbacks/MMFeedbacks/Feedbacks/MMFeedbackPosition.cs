...
public class MMFeedbackPosition : MMFeedback
{
    ...
    public AnimationCurve AnimatePositionCurve =
        new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.1f, 0.05f), new Keyframe(0.9f, 0.95f),
                           new Keyframe(1, 1));
    ...
    public AnimationCurve AnimatePositionCurveX =
        new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(0.6f, -1f), new Keyframe(1, 0f));
    ...
    {
        get { return ApplyTimeMultiplier(AnimatePositionDuration); }
    ...
    protected override void CustomInitialization(GameObject owner)
    {
        ...
        if (Active)
        {
            ...
            {
                Debug.LogWarning("The animate position target for "
                                 + this
    ...
    protected virtual void DeterminePositions()
    {
        ...
        {
            if (InitialPositionTransform != null) { InitialPosition = GetPosition(InitialPositionTransform); }
            else
            {
        ...
        if (Active && (AnimatePositionTarget != null))
        {
            if (isActiveAndEnabled || _hostMMFeedbacks.AutoPlayOnEnable)
            {
                ...
                case Modes.ToDestination:
                    ...
                    _coroutine = StartCoroutine(MoveFromTo(AnimatePositionTarget, _initialPosition,
                                                           _destinationPosition, FeedbackDuration,
                                                           AnimatePositionCurve));
                    break;
                case Modes.AtoB:
                    if (!AllowAdditivePlays && (_coroutine != null)) { return; }

                    _coroutine = StartCoroutine(MoveFromTo(AnimatePositionTarget, InitialPosition, DestinationPosition,
                                                           FeedbackDuration, AnimatePositionCurve));
                    break;
                case Modes.AlongCurve:
                    if (!AllowAdditivePlays && (_coroutine != null)) { return; }

                    float intensityMultiplier = Timing.ConstantIntensity ? 1f : feedbacksIntensity;
                    _coroutine = StartCoroutine(MoveAlongCurve(AnimatePositionTarget, InitialPosition, FeedbackDuration,
                                                               intensityMultiplier));
                    break;
                }
            }
    ...
    protected virtual IEnumerator MoveFromTo(GameObject movingObject, Vector3 pointA, Vector3 pointB, float duration,
        AnimationCurve curve = null)
    {
        ...
        // set final position
        if (NormalPlayDirection) { SetPosition(movingObject.transform, pointB); }
        else { SetPosition(movingObject.transform, pointA); }
