...
[AddComponentMenu("")]
[FeedbackHelp(
    "This feedback will instantiate the specified ParticleSystem at the specified position on Start or on Play, optionally nesting them.")]
...
public class MMFeedbackParticlesInstantiation : MMFeedback
{
    ...
    protected override void CustomInitialization(GameObject owner)
    {
        if (Active)
        {
            if (Mode == Modes.Cached) { InstantiateParticleSystem(); }
        }
    }
    ...
    protected virtual void InstantiateParticleSystem()
    {
        ...
        {
            if (_instantiatedParticleSystem != null) { Destroy(_instantiatedParticleSystem.gameObject); }
        }
        ...
        else { _instantiatedParticleSystem = GameObject.Instantiate(ParticlesPrefab) as ParticleSystem; }

        if (_instantiatedParticleSystem != null) { PositionParticleSystem(_instantiatedParticleSystem); }

        if ((_instantiatedRandomParticleSystems != null) && (_instantiatedRandomParticleSystems.Count > 0))
        {
            foreach (ParticleSystem system in _instantiatedRandomParticleSystems) { PositionParticleSystem(system); }
        }
    }
    ...
    protected virtual void PositionParticleSystem(ParticleSystem system)
    {
        ...
        {
            if (Owner != null) { InstantiateParticlesPosition = Owner.transform; }
        }
        ...
        if (NestParticles)
        {
            if (PositionMode == PositionModes.FeedbackPosition) { system.transform.SetParent(this.transform); }

            if (PositionMode == PositionModes.Transform) { system.transform.SetParent(InstantiateParticlesPosition); }
        }

        system.transform.position = GetPosition(this.transform.position);
        if (ApplyRotation) { system.transform.rotation = GetRotation(this.transform); }

        if (ApplyScale) { system.transform.localScale = GetScale(this.transform); }
    ...
    protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        ...
        if (Mode == Modes.OnDemand) { InstantiateParticleSystem(); }
    ...
    protected override void CustomStopFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        ...
        if (_instantiatedParticleSystem != null) { _instantiatedParticleSystem?.Stop(); }

        if ((_instantiatedRandomParticleSystems != null) && (_instantiatedRandomParticleSystems.Count > 0))
        {
            foreach (ParticleSystem system in _instantiatedRandomParticleSystems) { system.Stop(); }
        }
    }
