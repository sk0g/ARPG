...
[AddComponentMenu("")]
[FeedbackHelp(
    "This feedback will animate the target's rotation on the 3 specified animation curves (one per axis), for the specified duration (in seconds).")]
...
public class MMFeedbackRotation : MMFeedback
{
    ...
    [Tooltip("the angles to match when in ToDestination mode")] [MMFEnumCondition("Mode", (int) Modes.ToDestination)]
    ...
    {
        get { return ApplyTimeMultiplier(AnimateRotationDuration); }
        set { AnimateRotationDuration = value; }
    }
    ...
    protected override void CustomInitialization(GameObject owner)
    {
        ...
        if (Active && (AnimateRotationTarget != null)) { GetInitialRotation(); }
    }
    ...
    {
        _initialRotation = (RotationSpace == Space.World)
            ? AnimateRotationTarget.rotation
            : AnimateRotationTarget.localRotation;
        ...
        if (Active && (AnimateRotationTarget != null))
        {
            ...
            if (isActiveAndEnabled || _hostMMFeedbacks.AutoPlayOnEnable)
            {
                if ((Mode == Modes.Absolute) || (Mode == Modes.Additive))
                {
                    ...
                    _coroutine = StartCoroutine(AnimateRotation(AnimateRotationTarget, Vector3.zero, FeedbackDuration,
                                                                AnimateRotationX, AnimateRotationY, AnimateRotationZ,
    ...
    {
        if (AnimateRotationTarget == null) { yield break; }

        if ((AnimateRotationX == null) || (AnimateRotationY == null) || (AnimateRotationZ == null)) { yield break; }
        ...
        else { AnimateRotationTarget.transform.rotation = Quaternion.Euler(destinationAngles); }
    ...
    protected virtual void ApplyRotation(Transform targetTransform, float remapZero, float remapOne,
        AnimationCurve curveX, AnimationCurve curveY, AnimationCurve curveZ, float percent)
    {
        if (RotationSpace == Space.World) { targetTransform.transform.rotation = _initialRotation; }
        else { targetTransform.transform.localRotation = _initialRotation; }
