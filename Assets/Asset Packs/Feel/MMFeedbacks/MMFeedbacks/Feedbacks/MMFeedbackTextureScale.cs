...
public class MMFeedbackTextureScale : MMFeedback
{
    ...
    [Tooltip("the value to remap the scale curve's 0 to")] [MMFEnumCondition("Mode", (int) Modes.OverTime)]
    public Vector2 RemapZero = Vector2.zero;

    /// the value to remap the scale curve's 1 to
    [Tooltip("the value to remap the scale curve's 1 to")] [MMFEnumCondition("Mode", (int) Modes.OverTime)]
        ...
        if (Active)
        {
            ...
            case Modes.Instant:
                TargetRenderer.materials[MaterialIndex]
                              .SetTextureScale(MaterialPropertyName, InstantScale * intensityMultiplier);
                break;
            case Modes.OverTime:
