...
public class MMFeedbackTimescaleModifier : MMFeedback
{
    ...
    {
        get { return MMFeedbacksInspectorColors.TimeColor; }
    }
#endif
    ...
    [Tooltip("the duration of the timescale modification")] [MMFEnumCondition("Mode", (int) Modes.Shake)]
    public float TimeScaleDuration = 1f;

    /// whether or not we should lerp the timescale
    [Tooltip("whether or not we should lerp the timescale")] [MMFEnumCondition("Mode", (int) Modes.Shake)]
    public bool TimeScaleLerp = false;

    /// the speed at which to lerp the timescale
    [Tooltip("the speed at which to lerp the timescale")] [MMFEnumCondition("Mode", (int) Modes.Shake)]
    ...
    {
        get { return ApplyTimeMultiplier(TimeScaleDuration); }
        set { TimeScaleDuration = value; }
    }
    ...
    protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        if (Active)
        {
            ...
            {
            case Modes.Shake:
                MMTimeScaleEvent.Trigger(MMTimeScaleMethods.For, TimeScale, FeedbackDuration, TimeScaleLerp,
                                         TimeScaleLerpSpeed, false);
                break;
            case Modes.Change:
