...
public class MMFeedbackFlicker : MMFeedback
{
    ...
    [MMFEnumCondition("Mode", (int) Modes.PropertyName)] [Tooltip("the name of the property to target")]
    ...
    {
        get { return ApplyTimeMultiplier(FlickerDuration); }
    ...
    protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        if (Active && (BoundRenderer != null))
        {
            for (int i = 0; i < MaterialIndexes.Length; i++)
            {
                _coroutines[i] = StartCoroutine(Flicker(BoundRenderer, i, _initialFlickerColors[i], FlickerColor,
                                                        FlickerOctave, FeedbackDuration));
            }
        }
    }
    ...
    protected override void CustomReset()
    {
        ...
        if (Active && (BoundRenderer != null))
        {
            for (int i = 0; i < MaterialIndexes.Length; i++) { SetColor(i, _initialFlickerColors[i]); }
        }
    }

    public virtual IEnumerator Flicker(Renderer renderer, int materialIndex, Color initialColor, Color flickerColor,
        float flickerSpeed, float flickerDuration)
    {
        ...
        while (FeedbackTime < flickerStop)
        {
            ...
            else { yield return MMFeedbacksCoroutine.WaitForUnscaled(flickerSpeed); }
    ...
    protected override void CustomStopFeedback(Vector3 position, float feedbacksIntensity = 1)
    {
        ...
        if (Active)
        {
            ...
            {
                if (_coroutines[i] != null) { StopCoroutine(_coroutines[i]); }
