...
public class MMFeedbackTextureOffset : MMFeedback
{
    ...
    /// the material index
    [Tooltip("the material index")] public int MaterialIndex = 0;
    ...
    [Tooltip("how long the material should change over time")] [MMFEnumCondition("Mode", (int) Modes.OverTime)]
    ...
    [Tooltip("the value to remap the offset curve's 0 to")] [MMFEnumCondition("Mode", (int) Modes.OverTime)]
    public Vector2 RemapZero = Vector2.zero;

    /// the value to remap the offset curve's 1 to
    [Tooltip("the value to remap the offset curve's 1 to")] [MMFEnumCondition("Mode", (int) Modes.OverTime)]
        ...
        if (Active)
        {
            ...
            case Modes.Instant:
                TargetRenderer.materials[MaterialIndex]
                              .SetTextureOffset(MaterialPropertyName, InstantOffset * intensityMultiplier);
                break;
            case Modes.OverTime:
