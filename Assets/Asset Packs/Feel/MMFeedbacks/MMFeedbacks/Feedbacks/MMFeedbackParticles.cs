...
public class MMFeedbackParticles : MMFeedback
{
    ...
    protected virtual void PlayParticles(Vector3 position)
    {
        if (MoveToPosition)
        {
            ...
            foreach (ParticleSystem system in RandomParticleSystems) { system.transform.position = position; }
        }
    ...
    {
        foreach (ParticleSystem system in RandomParticleSystems) { system?.Stop(); }
