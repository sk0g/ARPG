...
[AddComponentMenu("")]
[FeedbackHelp(
    "This feedback will allow you to send to an animator (bound in its inspector) a bool, int, float or trigger parameter, allowing you to trigger an animation, with or without randomness.")]
...
public class MMFeedbackAnimation : MMFeedback
{
    ...
    [Tooltip("the selected mode to interact with this trigger")] [MMFCondition("UpdateTrigger", true)]
    ...
    /// the selected mode to interact with this trigger
    [Tooltip("the selected mode to interact with this trigger")] [MMFCondition("UpdateRandomTrigger", true)]
    ...
    [Tooltip("the bool parameter to turn true when the feedback gets played")] [MMFCondition("UpdateBool", true)]
    ...
    [Tooltip("the value to set to that int parameter")] [MMFEnumCondition("IntValueMode", (int) ValueModes.Constant)]
    public int IntValue;
    ...
    /// the float parameter to turn true when the feedback gets played
    [Tooltip("the float parameter to turn true when the feedback gets played")]
    [MMFEnumCondition("FloatValueMode", (int) ValueModes.Constant, (int) ValueModes.Random,
                      (int) ValueModes.Incremental)]
    ...
    protected override void CustomInitialization(GameObject owner)
    {
        ...
        foreach (string name in RandomBoolParameterNames) { _randomBoolParameters.Add(Animator.StringToHash(name)); }
    }
    ...
    protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        if (Active)
        {
            ...
            if (UpdateTrigger)
            {
                if (TriggerMode == TriggerModes.SetTrigger) { BoundAnimator.SetTrigger(_triggerParameter); }

                if (TriggerMode == TriggerModes.ResetTrigger) { BoundAnimator.ResetTrigger(_triggerParameter); }
            }
            ...
            if (UpdateRandomTrigger)
            {
                ...
                if (RandomTriggerMode == TriggerModes.SetTrigger) { BoundAnimator.SetTrigger(randomParameter); }

                if (RandomTriggerMode == TriggerModes.ResetTrigger) { BoundAnimator.ResetTrigger(randomParameter); }
            }

            if (UpdateBool) { BoundAnimator.SetBool(_boolParameter, BoolParameterValue); }
    ...
    {
        if (Active && UpdateBool) { BoundAnimator.SetBool(_boolParameter, false); }
    }
}
}