...
public class MMFeedbackRigidbody2D : MMFeedback
{
    ...
    [Tooltip("the min torque to apply to this rigidbody on play")] [MMFEnumCondition("Mode", (int) Modes.AddTorque)]
    public float MinTorque;

    /// the max torque to apply to this rigidbody on play
    [Tooltip("the max torque to apply to this rigidbody on play")] [MMFEnumCondition("Mode", (int) Modes.AddTorque)]
    public float MaxTorque;

    /// the force mode to apply
    [Tooltip("the force mode to apply")] public ForceMode2D AppliedForceMode = ForceMode2D.Impulse;
