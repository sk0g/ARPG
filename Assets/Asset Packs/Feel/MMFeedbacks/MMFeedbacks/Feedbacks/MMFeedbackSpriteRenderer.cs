...
public class MMFeedbackSpriteRenderer : MMFeedback
{
    ...
    [MMFEnumCondition("Mode", (int) Modes.OverTime, (int) Modes.ShakerEvent, (int) Modes.ToDestinationColor,
                      (int) Modes.ToDestinationColorAndBack)]
    ...
    [Tooltip("the channel to broadcast on")] [MMFEnumCondition("Mode", (int) Modes.ShakerEvent)]
    public int Channel = 0;

    /// whether or not to reset shaker values after shake
    [Tooltip("whether or not to reset shaker values after shake")] [MMFEnumCondition("Mode", (int) Modes.ShakerEvent)]
    ...
    [Tooltip("the color to move to in instant mode")]
    [MMFEnumCondition("Mode", (int) Modes.Instant, (int) Modes.ToDestinationColor,
                      (int) Modes.ToDestinationColorAndBack)]
    ...
    protected override void CustomInitialization(GameObject owner)
    {
        ...
        if (EventOriginTransform == null) { EventOriginTransform = this.transform; }
    ...
    protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        if (Active)
        {
            ...
            {
            case Modes.Instant:
                if (ModifyColor) { BoundSpriteRenderer.color = InstantColor; }
    ...
    protected virtual IEnumerator SpriteRendererToDestinationSequence(bool andBack)
    {
        ...
        if (ModifyColor) { BoundSpriteRenderer.color = andBack ? _initialColor : ToDestinationColor; }
    ...
    {
        if (ModifyColor) { BoundSpriteRenderer.color = ColorOverTime.Evaluate(time); }
    }
