...
public class MMBlink : MonoBehaviour
{
    ...
    [MMFEnumCondition("Method", (int) Methods.MaterialAlpha, (int) Methods.MaterialEmissionIntensity,
    ...
    [Tooltip("the state to apply on exit")] [MMFCondition("ForceStateOnExit", true)]
    ...
    /// Test button
    [Tooltip("Test button")] [MMFInspectorButton("StartBlinking")]
    public bool StartBlinkingButton;

    /// Test button
    [Tooltip("Test button")] [MMFInspectorButton("StopBlinking")]
    public bool StopBlinkingButton;

    /// is the blinking object in an active state right now?
    [Tooltip("is the blinking object in an active state right now?")] [MMFReadOnly]
    public bool Active = false;

    /// the index of the phase we're currently in
    [Tooltip("the index of the phase we're currently in")] [MMFReadOnly]
        ...
        if (CurrentPhaseIndex > Phases.Count - 1)
        {
            ...
            if (RepeatCount != -1)
            {
                ...
                if (_repeatCount < 0)
                {
                    ...
                    if (ForceStateOnExit)
                    {
                        if (StateOnExit == States.Off) { ApplyBlink(false, 0f); }
                        else { ApplyBlink(true, 1f); }
                    }
