...
{
    [Header("Status")] public bool WigglePermitted = true;
    ...
    [MMFReadOnly] public float LimitedTimeLeft;
...
public class MMWiggle : MonoBehaviour
{
    ...
    [Tooltip("the selected update mode")] public UpdateModes UpdateMode = UpdateModes.Update;
    ...
    protected virtual void WiggleValue(ref WiggleProperties property, ref InternalWiggleProperties internalProperties,
        float duration)
    {
    ...
    protected virtual void InitializeRandomValues(ref WiggleProperties properties,
        ref InternalWiggleProperties internalProperties)
    {
        ...
        RandomizeVector3(ref internalProperties.randomNoiseFrequency, properties.NoiseFrequencyMin,
                         properties.NoiseFrequencyMax);
        ...
        internalProperties.newValue = DetermineNewValue(properties, internalProperties.newValue,
                                                        internalProperties.initialValue,
                                                        ref internalProperties.startValue,
                                                        ref internalProperties.randomAmplitude,
                                                        ref internalProperties.randomFrequency,
                                                        ref internalProperties.pauseDuration);
    }
    ...
    {
        if (UpdateMode == UpdateModes.Update) { ProcessUpdate(); }
    }
    ...
    {
        if (UpdateMode == UpdateModes.LateUpdate) { ProcessUpdate(); }
    }
    ...
    {
        if (UpdateMode == UpdateModes.FixedUpdate) { ProcessUpdate(); }
    }
    ...
    protected virtual void ProcessUpdate()
    {
        ...
        if (UpdateValue(RotationActive, RotationWiggleProperties, ref _rotationInternalProperties))
        {
            transform.localEulerAngles =
                ApplyFalloff(_rotationInternalProperties.returnVector, RotationWiggleProperties);
        }
    ...
    /// <param name="internalProperties"></param>
    /// <returns></returns>
    protected virtual bool UpdateValue(bool valueActive, WiggleProperties properties,
        ref InternalWiggleProperties internalProperties)
    {
        ...
        case WiggleTypes.PingPong:
            return MoveVector3TowardsTarget(ref internalProperties.returnVector, properties,
                                            ref internalProperties.startValue, internalProperties.initialValue,
                                            ref internalProperties.newValue, ref internalProperties.timeSinceLastPause,
                                            ref internalProperties.timeSinceLastChange,
                                            ref internalProperties.randomAmplitude,
                                            ref internalProperties.randomFrequency,
    ...
    protected Vector3 ApplyFalloff(Vector3 newValue, WiggleProperties properties)
    {
        if ((properties.LimitedTime) && (properties.LimitedTimeTotal > 0f))
        {
            float curveProgress = (properties.LimitedTimeTotal - properties.LimitedTimeLeft)
                                  / properties.LimitedTimeTotal;
    ...
    /// <param name="properties"></param>
    /// <returns></returns>
    protected virtual Vector3 AnimateNoiseValue(ref InternalWiggleProperties internalProperties,
        WiggleProperties properties)
    {
        internalProperties.noiseElapsedTime += properties.GetDeltaTime();

        internalProperties.newValue.x =
            (Mathf.PerlinNoise(internalProperties.randomNoiseFrequency.x * internalProperties.noiseElapsedTime,
                               internalProperties.randomNoiseShift.x)
             * 2.0f
             - 1.0f)
            * internalProperties.randomAmplitude.x;
        internalProperties.newValue.y =
            (Mathf.PerlinNoise(internalProperties.randomNoiseFrequency.y * internalProperties.noiseElapsedTime,
                               internalProperties.randomNoiseShift.y)
             * 2.0f
             - 1.0f)
            * internalProperties.randomAmplitude.y;
        internalProperties.newValue.z =
            (Mathf.PerlinNoise(internalProperties.randomNoiseFrequency.z * internalProperties.noiseElapsedTime,
                               internalProperties.randomNoiseShift.z)
             * 2.0f
             - 1.0f)
            * internalProperties.randomAmplitude.z;

        if (properties.RelativeAmplitude) { internalProperties.newValue += internalProperties.initialValue; }
    ...
    /// <param name="properties"></param>
    /// <returns></returns>
    protected virtual Vector3 AnimateCurveValue(ref InternalWiggleProperties internalProperties,
        WiggleProperties properties)
    {
        ...
        if (internalProperties.timeSinceLastPause < internalProperties.pauseDuration)
        {
            ...
            EvaluateCurve(properties.Curve, curveProgress, internalProperties.remapZero, internalProperties.remapOne,
                          ref internalProperties.newValue);
            if (properties.RelativeCurveAmplitude) { internalProperties.newValue += internalProperties.initialValue; }
        }
        ...
        if (internalProperties.randomFrequency > 0)
        {
            ...
            if (internalProperties.curveDirection < 0f) { curveProgress = 1 - curveProgress; }
            ...
            if (internalProperties.timeSinceLastChange > internalProperties.randomFrequency)
            {
                ...
                RandomizeFloat(ref internalProperties.randomFrequency, properties.FrequencyMin,
                               properties.FrequencyMax);
            }
        }
    ...
    protected virtual void EvaluateCurve(AnimationCurve curve, float percent, Vector3 remapMin, Vector3 remapMax,
    ...
    /// <returns></returns>
    protected virtual bool MoveVector3TowardsTarget(ref Vector3 movedValue, WiggleProperties properties,
        ref Vector3 startValue, Vector3 initialValue,
        ...
        ref float pauseDuration, float frequency)
    {
        ...
        // handle pause
        if (timeSinceLastPause < pauseDuration) { return false; }

        // if we're just out of a pause
        if (timeSinceLastPause == timeSinceLastValueChange) { timeSinceLastValueChange = 0f; }
        ...
        if (frequency > 0)
        {
            ...
            if (!properties.UseSpeedCurve) { movedValue = Vector3.Lerp(startValue, destinationValue, curveProgress); }
            else
            {
    ...
    /// <param name="pauseDuration"></param>
    /// <returns></returns>
    protected virtual Vector3 DetermineNewValue(WiggleProperties properties, Vector3 newValue, Vector3 initialValue,
        ...
        ref Vector3 randomAmplitude, ref float randomFrequency, ref float pauseDuration)
    {
        ...
        case WiggleTypes.Random:
            ...
            if (properties.RelativeAmplitude) { newValue += initialValue; }
