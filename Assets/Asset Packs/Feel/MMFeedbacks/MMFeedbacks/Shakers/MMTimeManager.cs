...
{
    public delegate void Delegate(MMTimeScaleMethods timeScaleMethod, float timeScale, float duration, bool lerp,
    ...
    static public void Trigger(MMTimeScaleMethods timeScaleMethod, float timeScale, float duration, bool lerp,
        float lerpSpeed, bool infinite)
    {
...
{
    [MMFInformationAttribute(
        "Put this component in your scene and it'll catch MMFreezeFrameEvents and MMTimeScaleEvents, allowing you to control the flow of time.",
        MMFInformationAttribute.InformationType.Info, false)]
    ...
    [Tooltip("the current, real time, time scale")] [MMFReadOnly]
    public float CurrentTimeScale = 1f;

    /// the time scale the system is lerping towards
    [Tooltip("the time scale the system is lerping towards")] [MMFReadOnly]
    public float TargetTimeScale = 1f;

    /// whether or not the timescale should be lerping
    [Tooltip("whether or not the timescale should be lerping")] [MMFReadOnly]
    public bool LerpTimescale = true;

    /// the speed at which the timescale should lerp towards its target
    [Tooltip("the speed at which the timescale should lerp towards its target")] [MMFReadOnly]
    public float LerpSpeed;
    ...
    protected virtual void Update()
    {
        ...
        if (_timeScaleProperties.Count > 0)
        {
            ...
            if (_currentProperty.Duration <= 0f && !_currentProperty.Infinite) { Unfreeze(); }
        }
        else { TargetTimeScale = NormalTimescale; }
    ...
    protected virtual void ApplyTimeScale(float newValue)
    {
        ...
        if (newValue != 0) { Time.fixedDeltaTime = _initialFixedDeltaTime * newValue; }
    ...
    {
        if (_timeScaleProperties.Count > 0) { _timeScaleProperties.Pop(); }
        else { ResetTimeScale(); }
    }
    ...
    public virtual void OnTimeScaleEvent(MMTimeScaleMethods timeScaleMethod, float timeScale, float duration, bool lerp,
        float lerpSpeed, bool infinite)
    {
