...
public class MMShaker : MonoBehaviour
{
    ...
    [HideInInspector] internal bool _listeningToEvents = false;
    ...
    {
        if (_resetTargetValuesAfterShake) { ResetTargetValues(); }
        ...
        Vector3 eventOriginPosition = default(Vector3))
    {
        ...
        else
        {
            if (useRange)
            {
                if (Vector3.Distance(this.transform.position, eventOriginPosition) > range) { return false; }
            }
