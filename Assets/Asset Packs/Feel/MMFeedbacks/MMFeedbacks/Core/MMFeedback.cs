...
public abstract class MMFeedback : MonoBehaviour
{
    ...
    {
        get { return (Timing.CooldownDuration > 0f) && (FeedbackTime - _lastPlayTimestamp < Timing.CooldownDuration); }
    }
    ...
    public float FeedbackTime
    {
        get
        {
            if (Timing.TimescaleMode == TimescaleModes.Scaled) { return Time.time; }
    ...
    public float FeedbackDeltaTime
    {
        get
        {
            if (Timing.TimescaleMode == TimescaleModes.Scaled) { return Time.deltaTime; }
    ...
    public float TotalDuration
    {
        get
        {
            ...
            if (Timing.InitialDelay != 0) { totalTime += ApplyTimeMultiplier(Timing.InitialDelay); }
    ...
    {
        get { return ((FeedbackStartedAt > 0f) && (Time.time - FeedbackStartedAt < FeedbackDuration)); }
    }
    ...
    public virtual void Play(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        ...
        if (Timing.InitialDelay > 0f) { _playCoroutine = StartCoroutine(PlayCoroutine(position, feedbacksIntensity)); }
        else
        {
    ...
    protected virtual IEnumerator PlayCoroutine(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        ...
        else { yield return MMFeedbacksCoroutine.WaitForUnscaled(Timing.InitialDelay); }
    ...
    protected virtual void RegularPlay(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        ...
        if (Timing.UseIntensityInterval)
        {
            if ((feedbacksIntensity < Timing.IntensityIntervalMin)
                || (feedbacksIntensity >= Timing.IntensityIntervalMax)) { return; }
        }
        ...
        if (Timing.Sequence == null) { CustomPlayFeedback(position, feedbacksIntensity); }
        else { _sequenceCoroutine = StartCoroutine(SequenceCoroutine(position, feedbacksIntensity)); }
    }
    ...
    protected virtual IEnumerator InfinitePlay(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        while (true)
        {
            ...
            else
            {
                ...
                if (Timing.TimescaleMode == TimescaleModes.Scaled) { yield return MMFeedbacksCoroutine.WaitFor(delay); }
                else { yield return MMFeedbacksCoroutine.WaitForUnscaled(delay); }
            }
        }
    }
    ...
    protected virtual IEnumerator SequenceCoroutine(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        ...
        else
        {
            while (FeedbackTime - timeStartedAt < Timing.Sequence.Length)
            {
                ...
                {
                    if ((item.ID == Timing.TrackID)
                        && (item.Timestamp >= lastFrame)
    ...
    public virtual void Stop(Vector3 position, float feedbacksIntensity = 1.0f)
    {
        ...
        _playsLeft = Timing.NumberOfRepeats + 1;
        if (Timing.InterruptsOnStop) { CustomStopFeedback(position, feedbacksIntensity); }
    }
    ...
    public virtual void SetSequence(MMSequence newSequence)
    {
        ...
        if (Timing.Sequence != null)
        {
            for (int i = 0; i < Timing.Sequence.SequenceTracks.Count; i++)
            {
                if (Timing.Sequence.SequenceTracks[i].ID == Timing.TrackID) { _sequenceTrackID = i; }
            }
        }
    }
    ...
    {
        if (_isHostMMFeedbacksNotNull) { return _hostMMFeedbacks.ApplyTimeMultiplier(duration); }
