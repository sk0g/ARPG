...
public static class MMFeedbacksCoroutine
{
    ...
    {
        for (float timer = 0f; timer < seconds; timer += Time.deltaTime) { yield return null; }
    }
    ...
    {
        for (float timer = 0f; timer < seconds; timer += Time.unscaledDeltaTime) { yield return null; }
    }
}
}