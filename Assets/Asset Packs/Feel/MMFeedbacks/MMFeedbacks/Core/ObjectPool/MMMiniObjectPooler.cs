...
public class MMMiniObjectPooler : MonoBehaviour
{
    ...
    public virtual void FillObjectPool()
    {
        ...
        for (int i = 0; i < objectsToSpawn; i++) { AddOneObjectToThePool(); }
    }
    ...
    protected virtual GameObject AddOneObjectToThePool()
    {
        ...
        {
            Debug.LogWarning("The " + gameObject.name + " ObjectPooler doesn't have any GameObjectToPool defined.",
                             gameObject);
            return null;
        }
    ...
    {
        if (_waitingPool != null) { Destroy(_waitingPool.gameObject); }
    }
}
...
{
    [MMFReadOnly] public List<GameObject> PooledGameObjects;
}
}