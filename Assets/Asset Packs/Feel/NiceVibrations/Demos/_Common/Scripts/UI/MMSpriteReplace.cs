...
public class MMSpriteReplace : MonoBehaviour
{
    ...
    protected virtual void Initialization()
    {
        ...
        if (_mmTouchButton != null) { _mmTouchButton.ReturnToInitialSpriteAutomatically = false; }
        ...
        if (_image != null)
        {
            if (StartsOn) { _image.sprite = OnSprite; }
            else { _image.sprite = OffSprite; }
        }

        if (_spriteRenderer != null)
        {
            if (StartsOn) { _spriteRenderer.sprite = OnSprite; }
    ...
    public virtual void Swap()
    {
        if (_image != null)
        {
            if (_image.sprite != OnSprite) { SwitchToOnSprite(); }
            else { SwitchToOffSprite(); }
        }

        if (_spriteRenderer != null)
        {
            if (_spriteRenderer.sprite != OnSprite) { SwitchToOnSprite(); }
            else { SwitchToOffSprite(); }
        }
    }
    ...
    {
        if (_image != null) { _image.sprite = OffSprite; }

        if (_spriteRenderer != null) { _spriteRenderer.sprite = OffSprite; }
    }
