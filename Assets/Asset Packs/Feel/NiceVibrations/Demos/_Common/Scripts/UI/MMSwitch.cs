...
public class MMSwitch : MMTouchButton
{
    ...
    protected override void Update()
    {
        ...
        if (Time.time - _knobMovementStartedAt < KnobMovementDuration)
        {
            ...
            if (CurrentSwitchState == SwitchStates.Off)
            {
                SwitchKnob.transform.position =
                    Vector3.Lerp(OnPosition.transform.position, OffPosition.transform.position, value);
            }
            else
            {
                SwitchKnob.transform.position =
                    Vector3.Lerp(OffPosition.transform.position, OnPosition.transform.position, value);
            }
        }
    }
