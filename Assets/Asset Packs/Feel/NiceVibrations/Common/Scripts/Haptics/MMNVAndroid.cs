...
public static class MMNVAndroid
{
 ...
 AndroidJNIHelper.GetMethodID(AndroidVibrator.GetRawClass(), "vibrate", "(J)V", false);
    ...
    public static void AndroidVibrate(long milliseconds, int amplitude)
    {
        ...
        if ((AndroidSDKVersion() < 26)) { AndroidVibrate(milliseconds); }
        else
        {
    ...
    public static void AndroidVibrate(long[] pattern, int repeat)
    {
        ...
        if ((AndroidSDKVersion() < 26)) { AndroidVibrator.Call("vibrate", pattern, repeat); }
        else
        {
            AndroidVibrationEffectClassInitialization();
            try
    ...
    public static void AndroidVibrate(long[] pattern, int[] amplitudes, int repeat, bool threaded = false)
    {
        ...
        if ((pattern.Length == 0) || (amplitudes.Length == 0)) { return; }
        ...
        else
        {
            ...
            else { AndroidVibrateNoThread(pattern, amplitudes, repeat); }
        }
    }
