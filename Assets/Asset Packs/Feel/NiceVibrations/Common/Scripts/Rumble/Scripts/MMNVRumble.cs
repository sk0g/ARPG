...
public static class MMNVRumble
{
    ...
    public static UnityEngine.InputSystem.Gamepad GetGamepad(int id)
    {
        if (id != -1)
        {
            if (id >= UnityEngine.InputSystem.Gamepad.all.Count) { return UnityEngine.InputSystem.Gamepad.current; }
            else { return UnityEngine.InputSystem.Gamepad.all[id]; }
        }
    ...
    public static void Rumble(long[] pattern, int[] amplitudes, int repeat, MonoBehaviour coroutineSupport,
        int controllerID = -1)
    {
        if ((pattern == null) || (amplitudes == null)) { return; }

        coroutineSupport.StartCoroutine(
            RumblePatternCoroutine(pattern, amplitudes, amplitudes, repeat, coroutineSupport, controllerID));
    }
    ...
    {
        if ((pattern == null) || (lowFreqAmplitudes == null) || (highFreqAmplitudes == null)) { return; }

        coroutineSupport.StartCoroutine(RumblePatternCoroutine(pattern, lowFreqAmplitudes, highFreqAmplitudes, repeat,
        ...
        int[] highFreqAmplitudes, int repeat, MonoBehaviour coroutineSupport, int controllerID = -1)
    {
        ...
        while (currentIndex < pattern.Length)
        {
            ...
            do
            {
                ...
                float lowFrequencyAmplitude = (lowFreqAmplitudes.Length > currentIndex)
                    ? lowFreqAmplitudes[currentIndex] / 255f
                    : 0f;
                totalLowFrequency += lowFrequencyAmplitude;
                float highFrequencyAmplitude = (highFreqAmplitudes.Length > currentIndex)
                    ? highFreqAmplitudes[currentIndex] / 255f
                    : 0f;
            ...
            while (currentTime > Time.unscaledTime && currentIndex < pattern.Length) { yield return null; }
        }
