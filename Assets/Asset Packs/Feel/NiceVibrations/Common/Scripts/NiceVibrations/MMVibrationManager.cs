...
public static class MMVibrationManager
{
    ...
    public static void SetHapticsActive(bool status)
    {
        ...
        if (!status) { MMVibrationManager.StopContinuousHaptic(true); }
    }
    ...
    public static void Vibrate()
    {
        ...
        if (Android()) { MMNVAndroid.AndroidVibrate(MediumDuration); }
        else if (iOS())
        {
            ...
            else { MMNViOS.iOSTriggerHaptics(HapticTypes.MediumImpact); }
        }
    }
        ...
        MonoBehaviour coroutineSupport = null, int controllerID = -1)
    {
        ...
        else if (iOS()) { MMNViOS.iOSTriggerHaptics(type, defaultToRegularVibrate); }
    ...
    public static void TransientHaptic(float intensity, float sharpness, bool alsoRumble = false,
        MonoBehaviour coroutineSupport = null, int controllerID = -1)
    {
        TransientHaptic(true, intensity, sharpness, true, intensity, sharpness, true, alsoRumble, intensity, sharpness,
                        controllerID, coroutineSupport);
    }
        ...
        MonoBehaviour coroutineSupport = null, bool threaded = true)
    {
        ...
        {
            if (!MMNVAndroid.AndroidHasAmplitudeControl() && !vibrateAndroidIfNoSupport) { return; }
    ...
    public static void ContinuousHaptic(float intensity, float sharpness, float duration,
        ...
        int controllerID = -1, bool threaded = false, bool fullIntensity = true)
    {
        ContinuousHaptic(true, intensity, sharpness, fallbackOldiOS, true, intensity, sharpness, false, alsoRumble,
                         intensity, sharpness, controllerID, duration, mono, threaded, fullIntensity);
    }
    ...
    {
        UpdateContinuousHaptic(true, intensity, sharpness, true, intensity, sharpness, alsoRumble, intensity, sharpness,
    ...
    {
        AdvancedHapticPattern(true, iOSJSONString, true, androidPattern, androidAmplitudes, androidRepeat, false, true,
                              rumblePattern,
                              rumbleLowFreqAmplitudes, rumbleHighFreqAmplitudes, rumbleRepeat, fallbackOldiOS,
                              coroutineSupport, controllerID, threaded);
    }
    ...
    {
        if (_debugLogActive) { Debug.Log(log); }
    }
