...
public class MMAutoRotate : MonoBehaviour
{
    ...
    [MMCondition("Orbiting", true)] public Transform OrbitCenterTransform;

    /// the pivot (relative to the object's position in local space) to rotate around
    [MMCondition("Orbiting", true)] public Vector3 OrbitCenterOffset = Vector3.zero;

    /// the axis around which the object should rotate (don't make it zero)
    [MMCondition("Orbiting", true)] public Vector3 OrbitRotationAxis = new Vector3(0f, 1f, 0f);

    /// the speed at which to rotate
    [MMCondition("Orbiting", true)] public float OrbitRotationSpeed = 10f;

    /// the radius at which to orbit
    [MMCondition("Orbiting", true)] public float OrbitRadius = 3f;

    /// the speed at which the object catches up when orbit radius or axis changes
    [MMCondition("Orbiting", true)] public float OrbitCorrectionSpeed = 10f;
    ...
    [HideInInspector] public Vector3 _orbitCenter;
    [HideInInspector] public Vector3 _worldRotationAxis;
    [HideInInspector] public Plane _rotationPlane;
    [HideInInspector] public Vector3 _snappedPosition;
    [HideInInspector] public Vector3 _radius;
    ...
    {
        if (UpdateMode == UpdateModes.Update) { Rotate(); }
    }
    ...
    {
        if (UpdateMode == UpdateModes.FixedUpdate) { Rotate(); }
    }
    ...
    {
        if (UpdateMode == UpdateModes.LateUpdate) { Rotate(); }
    }
    ...
    {
        if (Rotating) { transform.Rotate(RotationSpeed * Time.deltaTime, RotationSpace); }
        ...
        if (Orbiting)
        {
            ...
            else { _worldRotationAxis = OrbitRotationAxis; }
            ...
            this.transform.position = Vector3.Lerp(this.transform.position, _desiredOrbitPosition,
                                                   OrbitCorrectionSpeed * Time.deltaTime);
