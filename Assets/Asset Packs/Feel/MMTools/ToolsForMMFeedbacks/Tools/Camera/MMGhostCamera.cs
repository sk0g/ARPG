...
public class MMGhostCamera : MonoBehaviour
{
    ...
    [Header("Virtual Joysticks")] public bool UseMobileControls;
    ...
    {
        if (AutoActivation) { ToggleFreeCamera(); }
    }
    ...
    {
        if (Input.GetKeyDown(ActivateButton)) { ToggleFreeCamera(); }
    ...
    protected virtual void GetInput()
    {
        ...
        if (Input.GetKey(UpButton)) { _currentInput.y = 1f; }

        if (Input.GetKey(DownButton)) { _currentInput.y = -1f; }

        _speedMultiplier = Input.GetKey(RunButton) ? RunFactor : 1f;
        _normalizedInput = _currentInput.normalized;

        if (Input.GetKeyDown(TimescaleModificationButton)) { ToggleSlowMotion(); }
    }
    ...
    {
        if (Input.GetKeyDown(ControlsModeSwitch)) { UseMobileControls = !UseMobileControls; }
        ...
        if (LeftStickContainer != null) { LeftStickContainer?.SetActive(UseMobileControls); }

        if (RightStickContainer != null) { RightStickContainer?.SetActive(UseMobileControls); }
    }
    ...
    protected virtual void Translate()
    {
        ...
        if ((Acceleration == 0) || (Deceleration == 0)) { _lerpedInput = _currentInput; }
        else
        {
    ...
    protected virtual void ToggleSlowMotion()
    {
        ...
        if (TimeAltered) { MMTimeScaleEvent.Trigger(MMTimeScaleMethods.For, TimescaleModifier, 1f, true, 5f, true); }
