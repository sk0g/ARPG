...
public class MMAutoRotateEditor : Editor
{
    ...
    static void DrawHandles(MMAutoRotate autoRotate, GizmoType gizmoType)
    {
        ...
        if (!myTarget.Orbiting || !myTarget.DrawGizmos) { return; }
        ...
        if (!Application.isPlaying)
        {
            if (myTarget.OrbitCenterTransform != null)
            {
                ...
                myTarget._worldRotationAxis =
                    myTarget.OrbitCenterTransform.TransformDirection(myTarget.OrbitRotationAxis);
                myTarget._rotationPlane.SetNormalAndPosition(myTarget._worldRotationAxis.normalized,
                                                             myTarget._orbitCenter);

                myTarget._snappedPosition = myTarget._rotationPlane.ClosestPointOnPlane(myTarget.transform.position);
                myTarget._radius = myTarget.OrbitRadius
                                   * Vector3.Normalize(myTarget._snappedPosition - myTarget._orbitCenter);
            }
        }
        ...
        Handles.DrawWireArc(myTarget._orbitCenter, myTarget._rotationPlane.normal,
                            Vector3.ProjectOnPlane(myTarget._orbitCenter + Vector3.forward,
                                                   myTarget._rotationPlane.normal), 360f, myTarget.OrbitRadius);
        ...
        else { direction = origin - (myTarget._orbitCenter + newRotation * myTarget._radius); }
