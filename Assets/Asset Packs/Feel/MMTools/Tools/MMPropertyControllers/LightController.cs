...
public class LightController : MonoBehaviour
{
    [Header("Binding")]
    [MMInformation(
        "Use this component to control the properties of one or more lights at runtime. Plays well with a FloatController. "
        + "This component will try to auto set the TargetLight if there's a Light component on this object.",
    ...
    {
        if (TargetLight == null) { TargetLight = this.gameObject.GetComponent<Light>(); }
    ...
    protected virtual void ApplyLightSettings()
    {
        ...
        if (TargetLights.Count > 0)
        {
            foreach (Light light in TargetLights)
            {
                if (light != null) { light.intensity = Intensity * Multiplier; }
            }
        }
    }
}
