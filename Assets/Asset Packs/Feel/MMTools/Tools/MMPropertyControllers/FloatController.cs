...
public class MonoAttribute
{
    ...
    public MonoAttribute(MonoBehaviour targetObject, MemberTypes type, PropertyInfo propertyInfo, FieldInfo fieldInfo,
        string memberName)
    {
    ...
    {
        if (MemberType == MonoAttribute.MemberTypes.Property) { MemberPropertyInfo.SetValue(TargetObject, newValue); }
        else if (MemberType == MonoAttribute.MemberTypes.Field) { MemberFieldInfo.SetValue(TargetObject, newValue); }
    }
}
...
public class FloatController : MMMonoBehaviour
{
    ...
    public virtual bool FindAttribute(string propertyName)
    {
        ...
        {
            Debug.LogError("FloatController "
                           + this.name
                           + " couldn't find any property or field named "
                           + propertyName
                           + " on "
                           + TargetObject.name);
            return false;
    ...
    protected virtual void Update()
    {
        ...
        case ControlModes.ToDestination:
            ...
            _remappedTimeSinceStart =
                MMMaths.Remap(GetTime() - _shakeStartTimestamp, 0f, ToDestinationDuration, 0f, 1f);
        ...
        if (ControlMode == ControlModes.ToDestination)
        {
            ...
            {
                _shaking = false;
                if (RevertToInitialValueAfterEnd) { CurrentValue = InitialValue; }
    ...
    public virtual void FillDropDownList()
    {
        ...
        {
            if (propertyInfo.PropertyType.Name == "Single") { _attributesNamesTempList.Add(propertyInfo.Name); }
        }

        _fieldReferences = TargetObject.GetType().GetFields();
        foreach (FieldInfo fieldInfo in _fieldReferences)
        {
            if (fieldInfo.FieldType.Name == "Single") { _attributesNamesTempList.Add(fieldInfo.Name); }
        }
