...
public class MMLineRendererDriver : MonoBehaviour
{
    ...
    protected virtual void Initialization()
    {
        ...
        if (!_countsMatch)
        {
            Debug.LogWarning(this.name
                             + ", MMLineRendererDriver's Targets list doesn't have the same amount of entries as the LineRender's Positions array. It won't work.");
        }
    }
    ...
    {
        if (BindPositionsToTargetsAtUpdate) { BindPositionsToTargets(); }
    }
    ...
    {
        if (!_countsMatch) { return; }

        for (int i = 0; i < Targets.Count; i++) { _lineRenderer.SetPosition(i, Targets[i].position); }
    }
