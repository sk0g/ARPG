...
public static class MMMaths
{
        ...
        float frequency, float speed, float deltaTime)
    {
        ...
        return velocity
               + (deltaTime * frequency * frequency * (targetValue - currentValue))
    ...
    public static Vector3 ProjectPointOnLine(Vector3 point, Vector3 lineStart, Vector3 lineEnd)
    {
        ...
        Vector3 lhs = vector2;
        if (magnitude > 1E-06f) { lhs = (Vector3) (lhs / magnitude); }
    ...
    {
        int result = 0;
        for (int i = 0; i < thingsToAdd.Length; i++) { result += thingsToAdd[i]; }
    ...
    {
        if (numberOfDecimals <= 0) { return Mathf.Round(value); }
        else { return Mathf.Round(value * 10f * numberOfDecimals) / (10f * numberOfDecimals); }
    }
    ...
    public static float RoundToClosest(float value, float[] possibleValues, bool pickSmallestDistance = false)
    {
        ...
        foreach (float possibleValue in possibleValues)
        {
            ...
            if (closestDistance > possibleDistance) { closestValue = possibleValue; }
