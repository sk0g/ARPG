...
public static class MMDebug
{
                                         ...
                                         m => m.GetTypes()
                                               .SelectMany(
                                                   n => n.GetMethods(
                                                             ...
                                                             | BindingFlags.Public)
                                                         .Where(o => o.GetCustomAttribute<MMDebugLogCommandAttribute>()
                                                                     != null)))
                                     .ToArray();
            }
    ...
    public static void DebugLogCommand(string command)
    {
        ...
        if (splitCommand.Length > 1)
        {
            ...
            {
                LogCommand(
                    "A version of command "
                    + commandFirst
                    + " with arguments could not be found. Maybe try without arguments.", "#ff2a00");
                return;
            }

            MMDebugLogCommandArgumentCountAttribute argumentAttribute = commandInfo
                                                                        .GetCustomAttributes<
                                                                            MMDebugLogCommandArgumentCountAttribute>(
                                                                            true)
                                                                        .FirstOrDefault();
            if (argumentAttribute != null && argumentAttribute.ArgumentCount > splitCommand.Length - 1)
            {
                LogCommand(
                    ...
                    + commandFirst
                    + " needs at least "
                    + argumentAttribute.ArgumentCount
                    + " arguments.", "#ff2a00");
                return;
            }
        ...
        else
        {
            ...
            if (commandInfo == null)
            {
                LogCommand("A version of command " + commandFirst + " without arguments could not be found.",
                           "#ff2a00");
                return;
            }
        }
    ...
    public struct DebugLogItem
    {
        ...
        public DebugLogItem(object message, string color, int framecount, float time, int timePrecision,
    ...
    public static string LogHistoryText
    {
        get
        {
            ...
            for (int i = 0; i < LogHistory.Count; i++)
            {
                ...
                log.Append("<color=#f9a682>["
                           + MMTime.FloatToTimeString(LogHistory[i].Time, false, true, true, true)
                           + "]</color> ");
    ...
    public static void DebugLogTime(object message, string color = "", int timePrecision = 3,
        bool displayFrameCount = true)
    {
        ...
        // build output
        string output = "";
        if (displayFrameCount) { output += "<color=#82d3f9>[f" + Time.frameCount + "]</color> "; }
        ...
        DebugLogItem item =
            new DebugLogItem(message, color, Time.frameCount, Time.time, timePrecision, displayFrameCount);

        // we add to our DebugLog 
        if (LogHistory.Count > _logHistoryMaxLength) { LogHistory.RemoveAt(0); }
    ...
    {
        if (drawGizmo && DebugDrawEnabled) { Debug.DrawRay(rayOriginPoint, rayDirection * rayDistance, color); }
        ...
        LayerMask mask, Color color, bool drawGizmo = false)
    {
        if (drawGizmo && DebugDrawEnabled)
        {
            ...
            points[4] = rotation
                        * ((origin + Vector2.left * halfSizeX + Vector2.up * halfSizeY)
                           + length * direction); // top left
            points[5] = rotation
                        * ((origin + Vector2.right * halfSizeX + Vector2.up * halfSizeY)
                           + length * direction); // top right
            points[6] = rotation
                        * ((origin + Vector2.right * halfSizeX - Vector2.up * halfSizeY)
                           + length * direction); // bottom right
            points[7] = rotation
                        * ((origin + Vector2.left * halfSizeX - Vector2.up * halfSizeY)
                           + length * direction); // bottom left
    ...
    public static RaycastHit2D MonoRayCastNonAlloc(RaycastHit2D[] array, Vector2 rayOriginPoint, Vector2 rayDirection,
        float rayDistance, LayerMask mask, Color color, bool drawGizmo = false)
    {
        if (drawGizmo && DebugDrawEnabled) { Debug.DrawRay(rayOriginPoint, rayDirection * rayDistance, color); }

        if (Physics2D.RaycastNonAlloc(rayOriginPoint, rayDirection, array, rayDistance, mask) > 0) { return array[0]; }
    ...
    /// <param name="color">Color.</param>
    /// <param name="drawGizmo">If set to <c>true</c> draw gizmo.</param>
    public static RaycastHit Raycast3D(Vector3 rayOriginPoint, Vector3 rayDirection, float rayDistance, LayerMask mask,
        Color color, bool drawGizmo = false)
    {
        if (drawGizmo && DebugDrawEnabled) { Debug.DrawRay(rayOriginPoint, rayDirection * rayDistance, color); }
    ...
    /// <param name="color">Color.</param>
    public static void DrawGizmoArrow(Vector3 origin, Vector3 direction, Color color, float arrowHeadLength = 3f,
    ...
    /// <param name="direction">Direction.</param>
    /// <param name="color">Color.</param>
    public static void DebugDrawArrow(Vector3 origin, Vector3 direction, Color color, float arrowHeadLength = 0.2f,
    ...
    public static void DebugDrawArrow(Vector3 origin, Vector3 direction, Color color, float arrowLength,
    ...
    /// <param name="arrowHeadLength">Arrow head length.</param>
    /// <param name="arrowHeadAngle">Arrow head angle.</param>
    private static void DrawArrowEnd(bool drawGizmos, Vector3 arrowEndPosition, Vector3 direction, Color color,
    ...
    public static void DrawHandlesBounds(Bounds bounds, Color color)
    {
        ...
        Vector3 v3FrontTopLeft = new Vector3(boundsCenter.x - boundsExtents.x, boundsCenter.y + boundsExtents.y,
                                             boundsCenter.z - boundsExtents.z); // Front top left corner
        Vector3 v3FrontTopRight = new Vector3(boundsCenter.x + boundsExtents.x, boundsCenter.y + boundsExtents.y,
                                              boundsCenter.z - boundsExtents.z); // Front top right corner
        Vector3 v3FrontBottomLeft = new Vector3(boundsCenter.x - boundsExtents.x, boundsCenter.y - boundsExtents.y,
                                                boundsCenter.z - boundsExtents.z); // Front bottom left corner
        Vector3 v3FrontBottomRight = new Vector3(boundsCenter.x + boundsExtents.x, boundsCenter.y - boundsExtents.y,
                                                 boundsCenter.z - boundsExtents.z); // Front bottom right corner
        Vector3 v3BackTopLeft = new Vector3(boundsCenter.x - boundsExtents.x, boundsCenter.y + boundsExtents.y,
                                            boundsCenter.z + boundsExtents.z); // Back top left corner
        Vector3 v3BackTopRight = new Vector3(boundsCenter.x + boundsExtents.x, boundsCenter.y + boundsExtents.y,
                                             boundsCenter.z + boundsExtents.z); // Back top right corner
        Vector3 v3BackBottomLeft = new Vector3(boundsCenter.x - boundsExtents.x, boundsCenter.y - boundsExtents.y,
                                               boundsCenter.z + boundsExtents.z); // Back bottom left corner
        Vector3 v3BackBottomRight = new Vector3(boundsCenter.x + boundsExtents.x, boundsCenter.y - boundsExtents.y,
    ...
    public static void DrawGizmoCube(Transform transform, Vector3 offset, Vector3 cubeSize, bool wireOnly)
    {
        ...
        if (wireOnly) { Gizmos.DrawWireCube(offset, cubeSize); }
        else { Gizmos.DrawCube(offset, cubeSize); }
    }
    ...
    public static string GetSystemInfo()
    {
        ...
        result += "\n<color=#FFFFFF>Graphic Device Name :</color> "
        ...
        if (logEntries != null)
        {
            MethodInfo clearMethod =
                logEntries.GetMethod(
                    "Clear", System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public);
