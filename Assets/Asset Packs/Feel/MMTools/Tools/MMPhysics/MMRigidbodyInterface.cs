...
public class MMRigidbodyInterface : MonoBehaviour
{
    ...
    public Vector3 position
    {
        get
        {
            if (_rigidbody2D != null) { return _rigidbody2D.position; }

            if (_rigidbody != null) { return _rigidbody.position; }
    ...
    public Vector3 Velocity
    {
        get
        {
            if (_mode == "2D") { return (_rigidbody2D.velocity); }
            else
            {
                if (_mode == "3D") { return (_rigidbody.velocity); }
                else { return new Vector3(0, 0, 0); }
            }
        }
        set
        {
            if (_mode == "2D") { _rigidbody2D.velocity = value; }

            if (_mode == "3D") { _rigidbody.velocity = value; }
        }
    }
    ...
    public Bounds ColliderBounds
    {
        get
        {
            if (_rigidbody2D != null) { return _collider2D.bounds; }

            if (_rigidbody != null) { return _collider.bounds; }
    ...
    public bool isKinematic
    {
        get
        {
            if (_mode == "2D") { return (_rigidbody2D.isKinematic); }

            if (_mode == "3D") { return (_rigidbody.isKinematic); }
    ...
    protected virtual void Awake()
    {
        ...
        {
            Debug.LogWarning(
                "A RigidBodyInterface has been added to "
                + gameObject
    ...
    {
        if (_mode == "2D") { _rigidbody2D.AddForce(force, ForceMode2D.Impulse); }

        if (_mode == "3D") { _rigidbody.AddForce(force); }
    }
    ...
    {
        if (_mode == "2D") { _rigidbody2D.AddRelativeForce(force, ForceMode2D.Impulse); }

        if (_mode == "3D") { _rigidbody.AddRelativeForce(force); }
    }
    ...
    {
        if (_mode == "2D") { _rigidbody2D.MovePosition(newPosition); }

        if (_mode == "3D") { _rigidbody.MovePosition(newPosition); }
    }
    ...
    {
        if (_mode == "2D") { _rigidbody2D.angularVelocity = 0; }

        if (_mode == "3D") { _rigidbody.angularVelocity = Vector3.zero; }
    }
    ...
    {
        if (_mode == "2D") { _rigidbody2D.rotation = 0; }

        if (_mode == "3D") { _rigidbody.rotation = Quaternion.identity; }
    }
    ...
    {
        if (_mode == "2D") { _rigidbody2D.isKinematic = status; }

        if (_mode == "3D") { _rigidbody.isKinematic = status; }
    }
    ...
    {
        if (_mode == "2D") { GetComponent<Collider2D>().enabled = status; }

        if (_mode == "3D") { GetComponent<Collider>().enabled = status; }
    }
