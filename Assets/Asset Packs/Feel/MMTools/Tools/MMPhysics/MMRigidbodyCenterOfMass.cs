...
public class MMRigidbodyCenterOfMass : MonoBehaviour
{
    ...
    [MMInspectorButton("SetCenterOfMass")] public bool SetCenterOfMassButton;
    ...
    protected virtual void Awake()
    {
        ...
        if (AutomaticSetMode == AutomaticSetModes.Awake) { SetCenterOfMass(); }
    }
    ...
    {
        if (AutomaticSetMode == AutomaticSetModes.Start) { SetCenterOfMass(); }
    }
    ...
    {
        if (_rigidbody != null) { _rigidbody.centerOfMass = CenterOfMassOffset; }

        if (_rigidbody2D != null) { _rigidbody2D.centerOfMass = CenterOfMassOffset; }

        if (AutoDestroyComponentAfterSet) { Destroy(this); }
    }
