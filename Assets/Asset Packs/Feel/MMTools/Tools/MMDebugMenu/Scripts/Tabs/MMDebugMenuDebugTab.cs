...
public class MMDebugMenuDebugTab : MonoBehaviour
{
    ...
    protected virtual void Awake()
    {
        ...
        {
            CommandPrompt.text = "";
            if (val != "") { MMDebug.DebugLogCommand(val); }
        });
    }
    ...
    protected virtual void Update()
    {
        ...
        if (TouchScreenVisible) { _rectTransform.MMSetBottom(650f); }
        else { _rectTransform.MMSetBottom(0f); }
    }
    ...
    {
        if (_touchScreenVisibleLastFrame != TouchScreenVisible) { StartCoroutine(ScrollToLogBottomCo()); }
    ...
    protected virtual void OnMMDebugLogEvent(MMDebug.DebugLogItem item)
    {
        ...
        if (this.gameObject.activeInHierarchy) { StartCoroutine(ScrollToLogBottomCo()); }
    }
