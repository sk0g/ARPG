...
public class MMDebugMenuItemSlider : MonoBehaviour
{
    ...
    [MMReadOnly] public float SliderValue;

    /// the current slider int value
    [MMReadOnly] public int SliderValueInt;
    ...
    public void ValueChangeCheck()
    {
        ...
        if (Mode == Modes.Int)
        {
            ...
            if (SliderValue == SliderValueInt) { valueChanged = false; }

            SliderValueInt = (int) SliderValue;
        }

        if (valueChanged) { UpdateValue(SliderValue); }
