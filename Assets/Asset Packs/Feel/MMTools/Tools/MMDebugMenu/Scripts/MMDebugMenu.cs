...
public class MMDebugMenu : MonoBehaviour
{
    ...
    [MMInspectorButton("ToggleMenu")] public bool ToggleButton;
    ...
    {
        int tabCounter = 0;
        if (MMLogo != null) { MMLogo.color = Data.TextColor; }

        foreach (Transform child in Contents.transform) { GameObject.Destroy(child.gameObject); }
    ...
    protected virtual void FillTab(MMDebugMenuTabContents tab, int index, bool triggerEvents = false)
    {
        ...
        foreach (MMDebugMenuItem item in Data.Tabs[index].MenuItems)
        {
            ...
            case MMDebugMenuItem.MMDebugMenuItemTypes.Button:
                ...
                button = (item.ButtonType == MMDebugMenuItem.MMDebugMenuItemButtonTypes.Border)
                    ? Instantiate(Data.ButtonBorderPrefab)
                    : Instantiate(Data.ButtonPrefab);
            ...
            case MMDebugMenuItem.MMDebugMenuItemTypes.Checkbox:
                ...
                if (item.CheckboxInitialState) { checkbox.Switch.SetTrue(); }
                else { checkbox.Switch.SetFalse(); }
            ...
            case MMDebugMenuItem.MMDebugMenuItemTypes.Slider:
                ...
                slider.TargetSlider.value =
                    MMMaths.Remap(item.SliderInitialValue, item.SliderRemapZero, item.SliderRemapOne, 0f, 1f);
                ...
                slider.SliderValueText.text = (item.SliderMode == MMDebugMenuItemSlider.Modes.Int)
                    ? item.SliderInitialValue.ToString()
                    : item.SliderInitialValue.ToString("F3");
            ...
            case MMDebugMenuItem.MMDebugMenuItemTypes.Spacer:
                GameObject spacerPrefab = (item.SpacerType == MMDebugMenuItem.MMDebugMenuItemSpacerTypes.Small)
                    ? Data.SpacerSmallPrefab
                    : Data.SpacerBigPrefab;
            ...
            case MMDebugMenuItem.MMDebugMenuItemTypes.Choices:
                ...
                else { choicesPrefab = Data.ThreeChoicesPrefab; }
                ...
                {
                    if (entry != null) { entry.ButtonText.font = Data.RegularFont; }
                }
    ...
    protected virtual IEnumerator ToggleCo(bool active)
    {
        ...
        MMTween.MoveRectTransform(this, _containerRect, _containerRect.localPosition, _newPosition, null, 0f,
                                  Data.ToggleDuration, Data.ToggleCurve, ignoreTimescale: true);
    ...
    {
        if (Input.GetKeyDown(Data.ToggleShortcut)) { ToggleMenu(); }
    }
}
}