...
{
    public static List<MMAchievement> AchievementsList
    {
    ...
    public static void LoadAchievementList()
    {
        ...
        foreach (MMAchievement achievement in achievementList.Achievements) { _achievements.Add(achievement.Copy()); }
    }
    ...
    public static void UnlockAchievement(string achievementID)
    {
        ...
        if (_achievement != null) { _achievement.UnlockAchievement(); }
    }
    ...
    {
        _achievement = AchievementManagerContains(achievementID);
        if (_achievement != null) { _achievement.LockAchievement(); }
    }
    ...
    {
        _achievement = AchievementManagerContains(achievementID);
        if (_achievement != null) { _achievement.AddProgress(newProgress); }
    }
    ...
    {
        _achievement = AchievementManagerContains(achievementID);
        if (_achievement != null) { _achievement.SetProgress(newProgress); }
    }
    ...
    {
        if (_achievements.Count == 0) { return null; }

        foreach (MMAchievement achievement in _achievements)
        {
            if (achievement.AchievementID == searchedID) { return achievement; }
        }
    ...
    {
        DeterminePath();
        SerializedMMAchievementManager serializedMMAchievementManager =
            (SerializedMMAchievementManager) MMSaveLoadManager.Load(typeof(SerializedMMAchievementManager),
    ...
    private static void DeterminePath(string specifiedFileName = "")
    {
        ...
        if (!string.IsNullOrEmpty(specifiedFileName)) { tempFileName = specifiedFileName; }
    ...
    public static void FillSerializedMMAchievementManager(SerializedMMAchievementManager serializedAchievements)
    {
        ...
        {
            SerializedMMAchievement newAchievement = new SerializedMMAchievement(
                _achievements[i].AchievementID, _achievements[i].UnlockedStatus, _achievements[i].ProgressCurrent);
    ...
    {
        if (serializedAchievements == null) { return; }
