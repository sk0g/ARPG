...
public class MMAim
{
    ...
    [Header("Control Mode")]
    [MMInformation(
        "Pick a control mode : mouse (aims towards the pointer), primary movement (you'll aim towards the current input direction), or secondary movement (aims "
        + "towards a second input axis, think twin stick shooters), and set minimum and maximum angles.",
    ...
    [MMReadOnly] public float CurrentAngle;
    ...
    public virtual Vector2 GetCurrentAim()
    {
        ...
        if ((CurrentAngle < MinimumAngle) || (CurrentAngle > MaximumAngle))
        {
            ...
            CurrentAngle = (Mathf.Abs(minAngleDifference) < Mathf.Abs(maxAngleDifference))
                ? MinimumAngle
                : MaximumAngle;
        }
