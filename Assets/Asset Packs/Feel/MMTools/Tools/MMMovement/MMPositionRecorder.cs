...
public class MMPositionRecorder : MonoBehaviour
{
    ...
    [MMReadOnly] public int FrameCounter;
    ...
    protected void Awake()
    {
        ...
        for (int i = 0; i < Positions.Length; i++) { Positions[i] = this.transform.position; }
    }
    ...
    {
        if (!RecordOnTimescaleZero && Time.timeScale == 0f) { return; }
    ...
    protected virtual void StorePositions()
    {
        ...
        {
            if (FrameCounter - _frameCountLastRecord < FrameInterval) { return; }

            _frameCountLastRecord = FrameCounter;
        }
        else
        {
            if (Time.time - _timeLastRecord < TimeInterval) { return; }
