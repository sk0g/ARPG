...
public class MMPreventPassingThrough : MonoBehaviour
{
    ...
    protected virtual void Initialization()
    {
        ...
        _smallestBoundsWidth = Mathf.Min(Mathf.Min(_collider.bounds.extents.x, _collider.bounds.extents.y),
    ...
    protected virtual void FixedUpdate()
    {
        ...
        if (_lastMovementSquared > _squaredBoundsWidth)
        {
            ...
            RaycastHit hitInfo;
            if (Physics.Raycast(_positionLastFrame, _lastMovement, out hitInfo, movementMagnitude,
                                ObstaclesLayerMask.value))
            {
                if (!hitInfo.collider) { return; }

                if (hitInfo.collider.isTrigger) { hitInfo.collider.SendMessage("OnTriggerEnter", _collider); }

                if (!hitInfo.collider.isTrigger)
                {
                    _rigidbody.position =
                        hitInfo.point - (_lastMovement / movementMagnitude) * _adjustedSmallestBoundsWidth;
                }
            }
        }
