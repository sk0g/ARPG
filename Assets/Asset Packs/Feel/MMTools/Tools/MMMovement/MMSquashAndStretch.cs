...
public class MMSquashAndStretch : MonoBehaviour
{
    ...
    [MMVector("Min", "Max")] public Vector2 SquashDuration = new Vector2(0.25f, 0.5f);

    /// the maximum intensity of the squash
    [MMVector("Min", "Max")] public Vector2 SquashIntensity = new Vector2(0f, 1f);
    ...
    {
        get { return (Timescale == Timescales.Regular) ? Time.time : Time.unscaledTime; }
    }

    public float TimescaleDeltaTime
    {
        get { return (Timescale == Timescales.Regular) ? Time.deltaTime : Time.unscaledDeltaTime; }
    }
    ...
    {
        if (TimescaleDeltaTime <= 0f) { return; }
    ...
    {
        if (VelocityMagnitude > 0.01f) { _newRotation = Quaternion.FromToRotation(Vector3.up, _direction); }
    ...
    protected virtual void ComputeNewLocalScale()
    {
        if (_squashing)
        {
            ...
            if (elapsed >= 1f) { _squashing = false; }
        }
        else
        {
