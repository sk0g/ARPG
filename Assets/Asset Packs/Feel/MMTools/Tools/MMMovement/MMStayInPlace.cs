...
public class MMStayInPlace : MonoBehaviour
{
    ...
    [Header("Overrides")] public bool OverridePosition = false;
    ...
    protected virtual void StayInPlace()
    {
        ...
        {
            if (FixedPosition) { this.transform.position = _initialPosition; }

            if (FixedRotation) { this.transform.rotation = _initialRotation; }
        }
        else
        {
            if (FixedPosition) { this.transform.localPosition = _initialPosition; }

            if (FixedRotation) { this.transform.localRotation = _initialRotation; }
        }

        if (FixedScale) { this.transform.localScale = _initialScale; }
    }
}
}