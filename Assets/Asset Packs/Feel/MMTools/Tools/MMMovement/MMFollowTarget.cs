...
public class MMFollowTarget : MonoBehaviour
{
    ...
    [MMCondition("FollowPosition", true)] public bool AddInitialDistanceXToXOffset = false;

    ///whether or not to add the initial y distance to the offset
    [MMCondition("FollowPosition", true)] public bool AddInitialDistanceYToYOffset = false;

    ///whether or not to add the initial z distance to the offset
    [MMCondition("FollowPosition", true)] public bool AddInitialDistanceZToZOffset = false;
    ...
    {
        get { return PositionSpace == PositionSpaces.Local; }
    }
    ...
    protected virtual void FollowTargetPosition()
    {
        ...
        if (InterpolatePosition)
        {
            ...
            case FollowModes.MMSpring:
                ...
                if (_localSpace) { this.transform.localPosition = _newPosition; }
                else { this.transform.position = _newPosition; }
            ...
            if (Vector3.Distance(this.transform.position, _initialPosition) > MaxDistanceToAnchor)
            {
                ...
                else
                {
                    this.transform.position = _initialPosition
                                              + Vector3.ClampMagnitude(this.transform.position - _initialPosition,
                                                                       MaxDistanceToAnchor);
                }
            }
        }
    ...
    protected virtual void FollowTargetRotation()
    {
        ...
        else { this.transform.rotation = _newTargetRotation; }
    }
    ...
    protected virtual void FollowTargetScale()
    {
        ...
        if (InterpolateScale)
        {
            ...
            case FollowModes.MMLerp:
                this.transform.localScale =
                    MMMaths.Lerp(this.transform.localScale, _newScale, FollowScaleSpeed, Time.deltaTime);
                break;
            case FollowModes.RegularLerp:
                this.transform.localScale =
                    Vector3.Lerp(this.transform.localScale, _newScale, Time.deltaTime * FollowScaleSpeed);
                break;
            case FollowModes.MMSpring:
                this.transform.localScale =
                    MMMaths.Lerp(this.transform.localScale, _newScale, FollowScaleSpeed, Time.deltaTime);
        ...
        else { this.transform.localScale = _newScale; }
    }
