...
public class MMFaceDirection : MonoBehaviour
{
    ...
    protected virtual void FaceDirection()
    {
        if (FacingMode == FacingModes.Target)
        {
            ...
            _direction =
                Quaternion.Euler(DirectionRotationAngles.x, DirectionRotationAngles.y, DirectionRotationAngles.z)
                * _direction;
            ApplyRotation();
        }
    ...
    {
        transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(_upwards, _direction),
                                              InterpolationSpeed * Time.time);
    }
