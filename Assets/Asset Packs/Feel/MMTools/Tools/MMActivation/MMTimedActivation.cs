...
public class MMTimedActivation : MonoBehaviour
{
    ...
    {
        if (ActivationMode == ActivationModes.Awake) { StartChangeState(); }
    }
    ...
    {
        if (ActivationMode == ActivationModes.Start) { StartChangeState(); }
    }
    ...
    protected virtual bool CorrectTagOrLayer(GameObject target)
    {
        ...
        case TriggerModes.Layer:
            if (((1 << target.layer) & TargetTriggerLayer) != 0) { return true; }
            else { return false; }
    ...
    {
        if (DelayMode == DelayModes.Time) { yield return MMCoroutine.WaitFor(TimeBeforeStateChange); }
        else { yield return StartCoroutine(MMCoroutine.WaitForFrames(FrameCount)); }
    ...
    {
        if (TimedActions != null) { TimedActions.Invoke(); }
    }
