...
public class MMAutoExecution : MonoBehaviour
{
    ...
    protected virtual void Awake()
    {
        ...
        {
            if ((item.AutoExecuteOnAwake) && (item.Event != null)) { item.Event.Invoke(); }
        }
    }
    ...
    protected virtual void Start()
    {
        foreach (MMAutoExecutionItem item in Events)
        {
            if ((item.AutoExecuteOnStart) && (item.Event != null)) { item.Event.Invoke(); }
        }
    }
    ...
    protected virtual void OnEnable()
    {
        foreach (MMAutoExecutionItem item in Events)
        {
            if ((item.AutoExecuteOnEnable) && (item.Event != null)) { item.Event.Invoke(); }
        }
    }
    ...
    protected virtual void OnDisable()
    {
        foreach (MMAutoExecutionItem item in Events)
        {
            if ((item.AutoExecuteOnDisable) && (item.Event != null)) { item.Event.Invoke(); }
        }
    }
    ...
    protected virtual void OnInstantiate()
    {
        foreach (MMAutoExecutionItem item in Events)
        {
            if ((item.AutoExecuteOnInstantiate) && (item.Event != null)) { item.Event.Invoke(); }
        }
    }
}
}