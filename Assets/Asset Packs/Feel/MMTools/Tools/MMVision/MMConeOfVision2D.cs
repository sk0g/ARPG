...
public class MMConeOfVision2D : MonoBehaviour
{
    ...
    [MMReadOnly] public List<Transform> VisibleTargets = new List<Transform>();

    [Header("Mesh")] public float MeshDensity = 0.2f;
    ...
    protected virtual void ScanForTargets()
    {
        ...
        foreach (Collider2D collider in _targetsWithinDistance)
        {
            ...
            if (Vector3.Angle(Direction, _directionToTarget) < VisionAngle / 2f)
            {
                ...
                RaycastHit2D hit2D = Physics2D.Raycast(this.transform.position, _directionToTarget, _distanceToTarget,
                                                       ObstacleMask);
                if (!hit2D) { VisibleTargets.Add(_target); }
            }
        }
    }
    ...
    RaycastData RaycastAtAngle(float angle)
    {
        ...
        if (hit2D) { return new RaycastData(true, hit2D.point, hit2D.distance, angle); }
        else { return new RaycastData(false, this.transform.position + direction * VisionRadius, VisionRadius, angle); }
    }
}
}