...
public class MMCountdown : MonoBehaviour
{
    ...
    [Header("Countdown")]
    [MMInformation(
        "You can define the bounds of the countdown (how much it should count down from, and to how much, the format it should be displayed in (standard Unity float ToString formatting).",
    ...
    [Header("Settings")]
    [MMInformation(
        "You can choose whether or not the countdown should automatically start on its Start, at what frequency (in seconds) it should refresh (0 means every frame), and the countdown's speed multiplier "
        ...
        + "Bind events here to trigger them when the countdown reaches its To destination, or every time it gets refreshed.",
        MoreMountains.Tools.MMInformationAttribute.InformationType.Info, false)]
    ...
    protected virtual void Initialization()
    {
        ...
        foreach (MMCountdownFloor floor in Floors) { floor.LastChangedAt = CountdownFrom; }
    }
    ...
    {
        if (Direction == MMCountdownDirections.Descending) { CurrentTime -= Time.deltaTime * CountdownSpeed; }
        else { CurrentTime += Time.deltaTime * CountdownSpeed; }
    }
    ...
    protected virtual void UpdateText()
    {
        if (Time.time - _lastRefreshAt > RefreshFrequency)
        {
            if (_text != null)
            {
                ...
                {
                    if (FloorValues) { newText = Mathf.Floor(CurrentTime).ToString(Format); }
                    else { newText = CurrentTime.ToString(Format); }
                }
                else { newText = MMTime.FloatToTimeString(CurrentTime, Hours, Minutes, Seconds, Milliseconds); }

                _text.text = newText;
            }

            if (CountdownRefreshEvent != null) { CountdownRefreshEvent.Invoke(); }
    ...
    protected virtual void CheckForEnd()
    {
        ...
        bool endReached = (Direction == MMCountdownDirections.Ascending)
            ? (CurrentTime >= CountdownTo)
            : (CurrentTime <= CountdownTo);

        if (endReached)
        {
            if (CountdownCompleteEvent != null) { CountdownCompleteEvent.Invoke(); }

            if (PingPong)
            {
                Direction = (Direction == MMCountdownDirections.Ascending)
    ...
    protected virtual void CheckForFloors()
    {
        foreach (MMCountdownFloor floor in Floors)
        {
            ...
            {
                if (floor.FloorEvent != null) { floor.FloorEvent.Invoke(); }
                ...
                if (Direction == MMCountdownDirections.Descending)
                {
                    ...
                    else { floor.LastChangedAt = floor.LastChangedAt - floor.FloorValue; }
                }
                else
                {
                    if (floor.LastChangedAt == CountdownFrom)
