...
public class MMLootTable<T, V> where T : MMLoot<V>
{
    ...
    [SerializeField] public List<T> ObjectsToLoot;

    /// the total amount of weights, for debug purposes only
    [Header("Debug")] [MMReadOnly] public float WeightsTotal;
    ...
    {
        if (ObjectsToLoot == null) { return; }

        if (ObjectsToLoot.Count == 0) { return; }
    ...
    {
        if (ObjectsToLoot == null) { return null; }

        if (ObjectsToLoot.Count == 0) { return null; }

        if (!_weightsComputed) { ComputeWeights(); }

        float index = Random.Range(0, WeightsTotal);

        foreach (T lootDropItem in ObjectsToLoot)
        {
            if ((index > lootDropItem.RangeFrom) && (index < lootDropItem.RangeTo)) { return lootDropItem; }
        }
