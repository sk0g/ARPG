...
public class MMPropertyLinkInt : MMPropertyLink
{
    ...
    public override float GetLevel(MMPropertyEmitter emitter, MMProperty property)
    {
        ...
        returnValue = MMMaths.Clamp(returnValue, emitter.IntRemapMinToZero, emitter.IntRemapMaxToOne, emitter.ClampMin,
                                    emitter.ClampMax);
    ...
    {
        if (_getterSetterInitialized) { SetIntDelegate(_newValue); }
