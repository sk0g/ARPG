...
public class MMPropertyLinkVector4 : MMPropertyLink
{
    ...
    public override void CreateGettersAndSetters(MMProperty property)
    {
        ...
        if (property.MemberType == MMProperty.MemberTypes.Property)
        {
            object firstArgument = (property.TargetScriptableObject == null)
                ? (object) property.TargetComponent
    ...
    public override void SetLevel(MMPropertyReceiver receiver, MMProperty property, float level)
    {
        ...
        _newValue.x = receiver.ModifyX
            ? MMMaths.Remap(level, 0f, 1f, receiver.Vector4RemapZero.x, receiver.Vector4RemapOne.x)
            : 0f;
        _newValue.y = receiver.ModifyY
            ? MMMaths.Remap(level, 0f, 1f, receiver.Vector4RemapZero.y, receiver.Vector4RemapOne.y)
            : 0f;
        _newValue.z = receiver.ModifyZ
            ? MMMaths.Remap(level, 0f, 1f, receiver.Vector4RemapZero.z, receiver.Vector4RemapOne.z)
            : 0f;
        _newValue.w = receiver.ModifyW
            ? MMMaths.Remap(level, 0f, 1f, receiver.Vector4RemapZero.w, receiver.Vector4RemapOne.w)
            : 0f;
    ...
    {
        if (_getterSetterInitialized) { SetVector4Delegate(_newValue); }
        else { SetPropertyValue(property, _newValue); }
    }
}
}