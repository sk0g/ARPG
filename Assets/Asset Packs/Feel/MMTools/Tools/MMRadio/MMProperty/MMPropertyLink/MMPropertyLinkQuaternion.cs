...
public class MMPropertyLinkQuaternion : MMPropertyLink
{
    ...
    public override float GetLevel(MMPropertyEmitter emitter, MMProperty property)
    {
        ...
        float returnValue = MMMaths.Remap(axisValue, emitter.QuaternionRemapMinToZero, emitter.QuaternionRemapMaxToOne,
                                          0f, 1f);
    ...
    {
        if (_getterSetterInitialized) { SetQuaternionDelegate(_newValue); }
