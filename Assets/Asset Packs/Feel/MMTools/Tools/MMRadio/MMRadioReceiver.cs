...
public class MMRadioReceiver : MMMonoBehaviour
{
    ...
    [MMCondition("CanListen", true)] public int Channel = 0;
    ...
    [MMCondition("RandomizeLevel", true)] public float MinRandomLevelMultiplier = 0f;

    /// if random, the max bound of the random multiplier
    [MMCondition("RandomizeLevel", true)] public float MaxRandomLevelMultiplier = 1f;
    ...
    protected virtual void Awake()
    {
        ...
        if (!_listeningToEvents && CanListen) { StartListening(); }
    ...
    protected virtual void OnRadioLevelEvent(int channel, float level)
    {
        ...
        if (RandomizeLevel) { level *= _randomLevelMultiplier; }
