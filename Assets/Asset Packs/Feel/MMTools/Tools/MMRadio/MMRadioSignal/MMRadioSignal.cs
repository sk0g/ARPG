...
public abstract class MMRadioSignal : MonoBehaviour
{
    ...
    {
        get { return (TimeScale == TimeScales.Scaled) ? Time.time : Time.unscaledTime; }
    }

    /// the delta time, unscaled or not
    public float TimescaleDeltaTime
    {
        get { return (TimeScale == TimeScales.Scaled) ? Time.deltaTime : Time.unscaledDeltaTime; }
    }
    ...
    [MMReadOnly] public float CurrentLevel = 0f;
    ...
    protected virtual void Update()
    {
        ...
        if (SignalMode == SignalModes.Driven) { ProcessDrivenMode(); }
    ...
    public virtual float ApplyBias(float t, float bias)
    {
        ...
        if (a < 0) { t = 1 - Mathf.Pow(1.0f - t, Mathf.Max(1 + a, .01f)); }
        else { t = Mathf.Pow(t, Mathf.Max(1 - a, .01f)); }
