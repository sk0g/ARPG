...
public class MMRadioSignalGenerator : MMRadioSignal
{
    ...
    [MMCondition("BackAndForth", true)] public float BackAndForthMirrorPoint = 0.5f;
    ...
    [MMVector("Min", "Max")] public Vector2 Clamps = new Vector2(0f, 1f);

    /// the amplitude of the signal
    [Range(0f, 1f)] public float Bias = 0.5f;
    ...
    {
        float level = 1f;
        if (SignalList.Count <= 0) { return level; }
        ...
        for (int i = 0; i < SignalList.Count; i++)
        {
            if (SignalList[i].Active)
            {
                float newLevel = MMSignal.GetValueNormalized(time,
                                                             ...
                                                             SignalList[i].Amplitude, SignalList[i].Frequency,
                                                             SignalList[i].Offset,
                                                             SignalList[i].Invert, SignalList[i].Curve,
                                                             SignalList[i].TweenCurve,
                                                             true, Clamps.x, Clamps.y, BackAndForth,
                                                             BackAndForthMirrorPoint);

                level = (SignalList[i].Mode == MMRadioSignalGeneratorItem.GeneratorItemModes.Multiply)
                    ? level * newLevel
                    : level + newLevel;
            }
        }
    ...
    protected override void Shake()
    {
        ...
        else { CurrentLevel = Evaluate(DriverTime); }
    }
...
public class MMRadioSignalGeneratorItem
{
    ...
    [Range(-1f, 1f)] public float Phase = 0f;

    /// the frequency of the signal
    [Range(0f, 10f)] public float Frequency = 5f;
