...
public class MMObjectBounds : MonoBehaviour
{
    ...
    [Header("Bounds")] public WaysToDetermineBounds BoundsBasedOn;
    ...
    protected virtual void DefineBoundsChoice()
    {
        ...
        if (GetComponent<Renderer>() != null) { BoundsBasedOn = WaysToDetermineBounds.Renderer; }

        if (GetComponent<Collider>() != null) { BoundsBasedOn = WaysToDetermineBounds.Collider; }

        if (GetComponent<Collider2D>() != null) { BoundsBasedOn = WaysToDetermineBounds.Collider2D; }
    }
    ...
    public virtual Bounds GetBounds()
    {
        if (BoundsBasedOn == WaysToDetermineBounds.Renderer)
        {
            ...
            {
                throw new Exception("The PoolableObject "
                                    + gameObject.name
