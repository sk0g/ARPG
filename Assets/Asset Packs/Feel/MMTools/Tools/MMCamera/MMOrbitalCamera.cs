...
public class MMOrbitalCamera : MonoBehaviour
{
    ...
    [MMReadOnly] public float DistanceToTarget = 5f;
    ...
    protected virtual void Rotation()
    {
        ...
        if (Mode == Modes.Touch && (Input.touchCount > 0))
        {
            if ((Input.touches[0].phase == TouchPhase.Moved) && (Input.touchCount == 1))
            {
                ...
                if (Input.touches[0].position.y < screenHeight / 4) { return; }
        ...
        if (Mode == Modes.Touch && (Input.touchCount > 0))
        {
            if (Input.touchCount == 2)
            {
                ...
                float previousTouchDeltaMagnitude =
                    (firstTouchPreviousPosition - secondTouchPreviousPosition).magnitude;
                ...
                _desiredDistance += deltaMagnitudeDifference
                                    * Time.deltaTime
                                    * ZoomSpeed
