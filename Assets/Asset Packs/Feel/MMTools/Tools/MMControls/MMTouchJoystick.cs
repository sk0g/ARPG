...
{
    [Header("Camera")] public Camera TargetCamera;

    [Header("Pressed Behaviour")]
    [MMInformation("Here you can set the opacity of the joystick when it's pressed. Useful for visual feedback.",
    ...
    [Header("Axis")]
    [MMInformation(
        "Choose if you want a joystick limited to one axis or not, and define the MaxRange. The MaxRange is the maximum distance from its initial center position you can drag the joystick to.",
        MMInformationAttribute.InformationType.Info, false)]
    ...
    [MMInformation(
        "And finally you can bind a function to get your joystick's values. Your method has to have a Vector2 as a parameter. Drag your object here and select the method.",
        MMInformationAttribute.InformationType.Info, false)]
    ...
    public virtual void Initialize()
    {
        ...
        SetNeutralPosition();
        if (TargetCamera == null) { throw new Exception("MMTouchJoystick : you have to set a target camera"); }
    ...
    protected virtual void Update()
    {
        ...
        {
            if (HorizontalAxisEnabled || VerticalAxisEnabled) { JoystickValue.Invoke(_joystickValue); }
        }
    ...
    {
        if (!_rotatingIndicatorIsNotNull) { return; }
    ...
    public virtual void OnDrag(PointerEventData eventData)
    {
        ...
        // otherwise
        else { _newTargetPosition = eventData.position; }
        ...
        if (!HorizontalAxisEnabled) { _newTargetPosition.x = 0; }

        if (!VerticalAxisEnabled) { _newTargetPosition.y = 0; }
