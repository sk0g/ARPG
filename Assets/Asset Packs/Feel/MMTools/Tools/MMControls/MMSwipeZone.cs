...
/// </summary>
public enum MMPossibleSwipeDirections
{
    Up,
    Down,
    Left,
...
public struct MMSwipeEvent
{
    ...
    public MMSwipeEvent(MMPossibleSwipeDirections direction, float angle, float length, Vector2 origin,
        Vector2 destination, float swipeDuration)
    {
    ...
    public static void Trigger(MMPossibleSwipeDirections direction, float angle, float length, Vector2 origin,
        Vector2 destination, float swipeDuration)
    {
...
public class MMSwipeZone : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IPointerExitHandler,
    IPointerEnterHandler
{
    ...
    protected virtual void Swipe()
    {
        ...
        MMSwipeEvent swipeEvent =
            new MMSwipeEvent(_swipeDirection, _angle, _length, _firstTouchPosition, _destination, duration);
        MMEventManager.TriggerEvent(swipeEvent);
        if (ZoneSwiped != null) { ZoneSwiped.Invoke(swipeEvent); }
    }

    protected virtual void Press()
    {
        if (ZonePressed != null) { ZonePressed.Invoke(); }
    }
    ...
    {
        if (Time.frameCount == _lastPointerUpAt) { return; }
        ...
        if (_deltaSwipe.magnitude < MaximumPressLength) { Press(); }
    ...
    {
        if (!MouseMode) { OnPointerDown(data); }
    }
    ...
    {
        if ((angle < 45) || (angle >= 315)) { return MMPossibleSwipeDirections.Right; }

        if ((angle >= 45) && (angle < 135)) { return MMPossibleSwipeDirections.Up; }

        if ((angle >= 135) && (angle < 225)) { return MMPossibleSwipeDirections.Left; }

        if ((angle >= 225) && (angle < 315)) { return MMPossibleSwipeDirections.Down; }
