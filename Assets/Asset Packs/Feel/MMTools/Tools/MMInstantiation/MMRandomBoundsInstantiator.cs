...
public class MMRandomBoundsInstantiator : MonoBehaviour
{
    ...
    [MMVector("Min", "Max")] public Vector2Int Quantity = new Vector2Int(1, 1);
    ...
    protected virtual void Awake()
    {
        ...
        if (StartMode == StartModes.Awake) { Instantiate(); }
    }
    ...
    {
        if (StartMode == StartModes.Start) { Instantiate(); }
    }
    ...
    {
        if (_instantiatedGameObjects == null) { _instantiatedGameObjects = new List<GameObject>(); }

        // we destroy our previous object if needed
        if (DestroyPreviouslyInstantiatedObjects)
        {
            foreach (GameObject go in _instantiatedGameObjects) { DestroyImmediate(go); }

            _instantiatedGameObjects.Clear();
        }

        int random = Random.Range(Quantity.x, Quantity.y);
        for (int i = 0; i < random; i++) { InstantiateRandomObject(); }
    }
    ...
    public virtual void InstantiateRandomObject()
    {
        ...
        if (ParentInstantiatedToThisObject) { obj.transform.SetParent(this.transform); }
