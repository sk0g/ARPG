...
public class MMRandomInstantiator : MonoBehaviour
{
    ...
    {
        if (StartMode == StartModes.Awake) { InstantiateRandomObject(); }
    }
    ...
    {
        if (StartMode == StartModes.Start) { InstantiateRandomObject(); }
    }
    ...
    public virtual void InstantiateRandomObject()
    {
        ...
        {
            if (_instantiatedGameObject != null) { DestroyImmediate(_instantiatedGameObject); }
        }

        // pick a random object and instantiates it
        int randomIndex = Random.Range(0, RandomPool.Count);
        _instantiatedGameObject =
            Instantiate(RandomPool[randomIndex], this.transform.position, this.transform.rotation);
        _instantiatedGameObject.name = InstantiatedObjectName;
        if (ParentInstantiatedToThisObject) { _instantiatedGameObject.transform.SetParent(this.transform); }
    }
}
}