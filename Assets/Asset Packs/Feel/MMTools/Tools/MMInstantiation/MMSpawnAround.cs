...
public class MMSpawnAroundProperties
{
    ...
    [Header("Shape")] [Tooltip("the shape within which objects should spawn")]
    ...
    [Tooltip("the minimum size of the cube's base")] [MMEnumCondition("Shape", (int) MMSpawnAroundShapes.Cube)]
    public Vector3 MinimumCubeBaseSize = Vector3.one;

    /// the maximum size of the cube's base
    [Tooltip("the maximum size of the cube's base")] [MMEnumCondition("Shape", (int) MMSpawnAroundShapes.Cube)]
    ...
    [Tooltip("whether or not to invert the curve (horizontally)")] [MMCondition("UseNormalAxisOffsetCurve", true)]
...
{
    public static void ApplySpawnAroundProperties(GameObject instantiatedObj, MMSpawnAroundProperties props,
        Vector3 origin)
    {
    ...
    public static Vector3 SpawnAroundPosition(MMSpawnAroundProperties props, Vector3 origin)
    {
        ...
        if (props.UseNormalAxisOffsetCurve)
        {
            ...
            if (randomOffset != 0)
            {
                ...
                {
                    normalizedOffset = MMMaths.Remap(randomOffset, props.MinimumNormalAxisOffset,
                                                     props.MaximumNormalAxisOffset, 1f, 0f);
                }
                else
                {
                    normalizedOffset = MMMaths.Remap(randomOffset, props.MinimumNormalAxisOffset,
                                                     props.MaximumNormalAxisOffset, 0f, 1f);
                }
            }
    ...
    public static void DrawGizmos(MMSpawnAroundProperties props, Vector3 origin, int quantity, float size,
        Color gizmosColor)
    {
