...
public class MMAnimationCurveGenerator : MonoBehaviour
{
    ...
    protected virtual void CreateAnimationCurve(ScriptableObject asset, MMTween.MMTweenCurve curveType,
        int curveResolution, bool anti)
    {
        ...
        for (int i = 0; i < curveResolution; i++)
        {
            ...
            if (anti) { _keyframe.value = MMTween.Tween(_keyframe.time, 0f, 1f, 1f, 0f, curveType); }
            else { _keyframe.value = MMTween.Tween(_keyframe.time, 0f, 1f, 0f, 1f, curveType); }

            animationCurve.AddKey(_keyframe);
        }

        // smoothes the curve's tangents
        for (int j = 0; j < curveResolution; j++) { animationCurve.SmoothTangents(j, 0f); }
