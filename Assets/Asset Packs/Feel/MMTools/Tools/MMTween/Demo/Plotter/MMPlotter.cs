...
public class MMPlotter : MonoBehaviour
{
    ...
    [Header("Graph")] public float GraphSize = 1f;
    [Range(0, 1000)] public int Resolution = 100;

    [Header("Points")] public Transform PlotPointPrefab;
    public float PointScaleFactor = 1f;
    public Material PlotPointMaterial;
    [MMReadOnly] public float DistanceBetweenPoints = 1f;

    [Header("Axis")] public MMPlotterAxis Axis;
    ...
    public virtual string[] GetMethodsList()
    {
        ...
        foreach (MethodInfo method in _methodList) { methodNames.Add(method.Name); }
    ...
    {
        if (_methodList == null) { FillMethodList(); }
    ...
    [Header("Movement")] public float MovementPauseDuration = 0.5f;
    ...
    protected virtual void Update()
    {
        ...
        if (Time.unscaledTime - _lastMovementEndedAt < MovementPauseDuration)
        {
            ...
            {
                _currentMovement = 1f;
                _newScaleUnit = MMTween.Tween(Time.unscaledTime - _lastMovementEndedAt, 0f,
                                              (MovementPauseDuration / 2f), 1f, 0f, MMTween.MMTweenCurve.EaseInCubic);
            ...
            {
                _currentMovement = 0f;
                _newScaleUnit = MMTween.Tween(Time.unscaledTime - _lastMovementEndedAt, (MovementPauseDuration / 2f),
                                              MovementPauseDuration, 0f, 1f, MMTween.MMTweenCurve.EaseOutCubic);
                _newScale = Vector3.one * _newScaleUnit;
