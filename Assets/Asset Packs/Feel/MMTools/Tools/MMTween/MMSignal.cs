...
public class MMSignal
{
    ...
    public static float GetValue(float time, SignalType signalType, float phase, float amplitude, float frequency,
        float offset, bool Invert = false, AnimationCurve curve = null,
    ...
    public static float GetValueNormalized(float time, SignalType signalType,
        ...
        bool clamp = true, float clampMin = 0f, float clampMax = 1f, bool backAndForth = false,
        float backAndForthTippingPoint = 0.5f)
    {
        ...
        if (backAndForth)
        {
            if (time < backAndForthTippingPoint) { time = MMMaths.Remap(time, 0f, backAndForthTippingPoint, 0f, 1f); }
            else if (time == backAndForthTippingPoint) { time = 1f; }
        ...
        if (Invert) { value = MMMaths.Remap(value, 0f, 1f, 1f, 0f); }

        float returnValue = amplitude * value + offset;

        // we clamp the value
        if (clamp) { returnValue = Mathf.Clamp(returnValue, clampMin, clampMax); }
