...
public class AIBrain : MonoBehaviour
{
    ...
    {
        foreach (AIState state in States) { state.SetBrain(this); }
    ...
    protected virtual void InitializeDecisions()
    {
        ...
        foreach (AIDecision decision in _decisions) { decision.Initialization(); }
    }
    ...
    protected AIState FindState(string stateName)
    {
        ...
        if (stateName != "")
        {
            Debug.LogError("You're trying to transition to state '"
                           + stateName
                           + "' in "
