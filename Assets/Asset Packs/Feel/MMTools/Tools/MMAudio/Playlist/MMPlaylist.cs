...
public class MMPlaylistSong
{
    ...
    [MMVector("Min", "Max")] public Vector2 Volume = new Vector2(0f, 1f);
    ...
    [MMVector("RMin", "RMax")] public Vector2 CrossFadeDuration = new Vector2(2f, 2f);
    ...
    [MMReadOnly] public bool Playing = false;

    /// whether this song is fading right now or not
    [MMReadOnly] public bool Fading = false;
...
public class MMPlaylist : MonoBehaviour
{
    ...
    [MMReadOnly] public int CurrentlyPlayingIndex = -1;

    /// the name of the track that is currently playing
    [MMReadOnly] public string CurrentTrackName;
    ...
    protected virtual IEnumerator PlayTrack(int index)
    {
        ...
        if (CurrentlyPlayingIndex >= 0)
        {
            ...
            {
                if (song != Songs[CurrentlyPlayingIndex]) { song.Fading = false; }
            }
        }
    ...
    protected virtual int PickNextIndex()
    {
        ...
        else { newIndex = (CurrentlyPlayingIndex + 1) % Songs.Count; }
    ...
    protected virtual void OnValidate()
    {
        ...
        else
        {
            if (Songs.Count != _listCount)
            {
                if (Songs.Count > _listCount)
                {
                    foreach (MMPlaylistSong song in Songs) { song.Initialization(); }
                }
