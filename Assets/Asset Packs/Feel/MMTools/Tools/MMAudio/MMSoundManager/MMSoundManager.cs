    ...
    MMEventListener<MMSoundManagerTrackFadeEvent>
{
    ...
    {
        if ((settingsSo != null) && (settingsSo.Settings.AutoLoad)) { settingsSo.LoadSoundSettings(); }
    }
    ...
    {
        if (_pool == null) { _pool = new MMSoundManagerAudioPool(); }
    ...
    public virtual AudioSource PlaySound(AudioClip audioClip, MMSoundManagerPlayOptions options)
    {
        return PlaySound(audioClip, options.MmSoundManagerTrack, options.Location,
                         ...
                         options.DopplerLevel, options.Spread, options.RolloffMode, options.MinDistance,
                         options.MaxDistance
        );
    }
    ...
    )
    {
        ...
        if (!loop && !recycleAudioSource) { Destroy(_tempAudioSourceGameObject, audioClip.length); }
        ...
        if (soloSingleTrack)
        {
            ...
            if (autoUnSoloOnEnd) { MuteSoundsOnTrack(mmSoundManagerTrack, false, audioClip.length); }
        }
        else if (soloAllTracks)
        {
            MuteAllSounds();
            audioSource.mute = false;
            if (autoUnSoloOnEnd) { StartCoroutine(MuteAllSoundsCoroutine(audioClip.length, false)); }
        }
    ...
    {
        source.Stop();
        if (!_pool.FreeSound(source)) { Destroy(source.gameObject); }
    }
    ...
    public virtual float GetTrackVolume(MMSoundManagerTracks track, bool mutedVolume)
    {
        ...
        {
        case MMSoundManagerTracks.Master:
            if (mutedVolume) { return settingsSo.Settings.MutedMasterVolume; }
            else { return settingsSo.Settings.MasterVolume; }
        case MMSoundManagerTracks.Music:
            if (mutedVolume) { return settingsSo.Settings.MutedMusicVolume; }
            else { return settingsSo.Settings.MusicVolume; }
        case MMSoundManagerTracks.Sfx:
            if (mutedVolume) { return settingsSo.Settings.MutedSfxVolume; }
            else { return settingsSo.Settings.SfxVolume; }
        case MMSoundManagerTracks.UI:
            if (mutedVolume) { return settingsSo.Settings.MutedUIVolume; }
            else { return settingsSo.Settings.UIVolume; }
        }
    ...
    protected virtual void ControlTrack(MMSoundManagerTracks track, ControlTrackModes trackMode, float volume = 0.5f)
    {
        ...
        if (settingsSo.Settings.AutoSave) { settingsSo.SaveSoundSettings(); }
    }
    ...
    /// <param name="tweenType"></param>
    /// <returns></returns>
    protected virtual IEnumerator FadeTrackCoroutine(MMSoundManagerTracks track, float duration, float initialVolume,
        float finalVolume, MMTweenType tweenType)
    {
        float startedAt = Time.unscaledTime;
        if (tweenType == null) { tweenType = new MMTweenType(MMTween.MMTweenCurve.EaseInOutQuartic); }
    ...
    {
        if (delay > 0) { yield return MMCoroutine.WaitForUnscaled(delay); }
    ...
    protected virtual IEnumerator MuteAllSoundsCoroutine(float delay, bool mute = true)
    {
        ...
        foreach (MMSoundManagerSound sound in _sounds) { sound.Source.mute = mute; }
    }
    ...
    {
        foreach (MMSoundManagerSound sound in _sounds) { sound.Source.Pause(); }
    }
    ...
    {
        foreach (MMSoundManagerSound sound in _sounds) { sound.Source.Play(); }
    }
    ...
    {
        foreach (MMSoundManagerSound sound in _sounds) { sound.Source.Stop(); }
    }
    ...
    public virtual void FreeAllLoopingSounds()
    {
        foreach (MMSoundManagerSound sound in _sounds)
        {
            if ((sound.Source.loop) && (sound.Source != null)) { FreeSound(sound.Source); }
        }
    }
    ...
    {
        if (soundControlEvent.TargetSource == null) { _tempAudioSource = FindByID(soundControlEvent.SoundID); }
        else { _tempAudioSource = soundControlEvent.TargetSource; }
    ...
    {
        FadeTrack(trackFadeEvent.Track, trackFadeEvent.FadeDuration, settingsSo.GetTrackVolume(trackFadeEvent.Track),
                  trackFadeEvent.FinalVolume, trackFadeEvent.FadeTween);
    }
