...
public class Beat
{
    ...
    [MMEnumCondition("Mode", (int) Modes.Raw, (int) Modes.Normalized, (int) Modes.BufferedRaw,
                     (int) Modes.BufferedNormalized)]
    ...
    [MMReadOnly] public bool BeatThisFrame;
    [MMReadOnly] public float CurrentValue;
    [HideInInspector] public float _previousValue;
    [HideInInspector] public float _lastBeatAt;
    [HideInInspector] public float _lastBeatValue;
...
public class MMAudioAnalyzer : MonoBehaviour
{
    ...
    [MMInformation(
        ...
        + "will determine in how many bands you want to cut the sound. The more bands, the more levers you'll have to play with afterwards."
        + "In general, for all of these settings, higher values mean better quality and lower performance. The buffer speed determines how "
        + "fast buffered band levels readjust.", MoreMountains.Tools.MMInformationAttribute.InformationType.Info,
        false)]
    [MMReadOnlyWhenPlaying]
    public Modes Mode = Modes.Global;

    [MMEnumCondition("Mode", (int) Modes.AudioSource)] [MMReadOnlyWhenPlaying]
    public AudioSource TargetAudioSource;

    [MMEnumCondition("Mode", (int) Modes.Microphone)]
    public int MicrophoneID = 0;

    [Header("Sampling")] [MMReadOnlyWhenPlaying]
    public float SampleInterval = 0.02f;

    [MMDropdown(2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192)] [MMReadOnlyWhenPlaying]
    public int SpectrumSamples = 1024;

    [MMReadOnlyWhenPlaying] public FFTWindow Window = FFTWindow.Rectangular;
    [Range(1, 64)] [MMReadOnlyWhenPlaying] public int NumberOfBands = 8;
    public float BufferSpeed = 2f;

    [Header("Beat Events")] public Beat[] Beats;

    [HideInInspector] public float[] RawSpectrum;

    [HideInInspector] public float[] BandLevels;
    [HideInInspector] public float[] BufferedBandLevels;

    [HideInInspector] public float[] BandPeaks;
    [HideInInspector] public float[] LastPeaksAt;

    [HideInInspector] public float[] NormalizedBandLevels;
    [HideInInspector] public float[] NormalizedBufferedBandLevels;

    [HideInInspector] public float Amplitude;
    [HideInInspector] public float NormalizedAmplitude;
    [HideInInspector] public float BufferedAmplitude;
    [HideInInspector] public float NormalizedBufferedAmplitude;
    [HideInInspector] public bool Active = false;
    [HideInInspector] public bool PeaksPasted = false;
        ...
        while (true)
        {
            ...
            case Modes.Microphone:
                ...
                if (microphoneSamples / _microphoneFrequency > _microphoneDelay)
                {
                    ...
                    {
                        TargetAudioSource.timeSamples =
                            (int) (microphoneSamples - (_microphoneDelay * _microphoneFrequency));
    ...
    protected virtual void HandleBuffer()
    {
        ...
        {
            BufferedBandLevels[i] = Mathf.Max(BufferedBandLevels[i] * Mathf.Exp(-BufferSpeed * Time.deltaTime),
                                              BandLevels[i]);
    ...
    protected virtual void HandleBeats()
    {
        ...
        foreach (Beat beat in Beats)
        {
            ...
            if (beat.BeatValueMode == Beat.BeatValueModes.Live) { beat.CurrentValue = value; }
            else
            {
                ...
                if ((beat._previousValue > beat.Threshold) && (value <= beat.Threshold))
                {
                    ...
                    if (Time.time - beat._lastBeatAt > beat.MinimumTimeBetweenBeats) { OnBeat(beat, value); }
                }
    ...
    protected virtual void OnBeat(Beat beat, float rawValue)
    {
        ...
        if (beat.OnBeat != null) { beat.OnBeat.Invoke(); }
    ...
    protected virtual IEnumerator RemapBeat(Beat beat)
    {
        ...
        while (Time.time - remapStartedAt < beat.RemappedAttack + beat.RemappedDecay)
        {
            ...
            {
                beat.CurrentValue =
                    Mathf.Lerp(1f, 0f, (Time.time - remapStartedAt - beat.RemappedAttack) / beat.RemappedDecay);
            }
    ...
    protected virtual void OnValidate()
    {
        ...
        {
            if (bandCounter >= _cachedNumberOfBands) { bandCounter = 0; }
