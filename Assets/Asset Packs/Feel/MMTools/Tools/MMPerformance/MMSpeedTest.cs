...
public static class MMSpeedTest
{
    ...
    {
        if (_speedTests.ContainsKey(testID)) { _speedTests.Remove(testID); }
    ...
    {
        if (!_speedTests.ContainsKey(testID)) { return; }
        ...
        UnityEngine.Debug.Log("<color=red>MMSpeedTest</color> [Test "
                              + testID
                              + "] test duration : "
                              + elapsedTime
                              + "s");
