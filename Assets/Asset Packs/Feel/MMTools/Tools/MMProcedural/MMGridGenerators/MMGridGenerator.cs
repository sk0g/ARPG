...
public class MMGridGenerator
{
    ...
    public static int[,] InvertGrid(int[,] grid)
    {
        ...
        {
            for (int j = 0; j <= grid.GetUpperBound(1); j++) { grid[i, j] = grid[i, j] == 0 ? 1 : 0; }
        }
    ...
    public static int[,] SmoothenGrid(int[,] grid)
    {
        ...
        for (int i = 0; i <= width; i++)
        {
            for (int j = 0; j <= height; j++)
            {
                ...
                if (adjacentWallsCount > 4) { grid[i, j] = 1; }
        ...
        List<MMTilemapGeneratorLayer.MMTilemapGeneratorLayerSafeSpot> safeSpots)
    {
        foreach (MMTilemapGeneratorLayer.MMTilemapGeneratorLayerSafeSpot safeSpot in safeSpots)
        {
            ...
            {
                for (int j = minY; j < maxY; j++) { SetGridCoordinate(grid, i, j, 0); }
            }
        }
    ...
    public static int[,] BindGrid(int[,] grid, bool top, bool bottom, bool left, bool right)
    {
        ...
        if (top)
        {
            for (int i = 0; i <= width; i++) { grid[i, height] = 1; }
        }

        if (bottom)
        {
            for (int i = 0; i <= width; i++) { grid[i, 0] = 1; }
        }

        if (left)
        {
            for (int j = 0; j <= height; j++) { grid[0, j] = 1; }
        }

        if (right)
        {
            for (int j = 0; j <= height; j++) { grid[width, j] = 1; }
        }
