...
public class MMFaderRound : MonoBehaviour, MMEventListener<MMFadeEvent>, MMEventListener<MMFadeInEvent>,
    MMEventListener<MMFadeOutEvent>, MMEventListener<MMFadeStopEvent>
{
    ...
    [Header("Bindings")] public CameraModes CameraMode = CameraModes.Main;
    ...
    [Header("Debug")] public Transform DebugWorldPositionTarget;
    [MMInspectorButton("FadeIn1Second")] public bool FadeIn1SecondButton;
    ...
    {
        MMFadeEvent.Trigger(DefaultDuration, MaskScale.y, DefaultTween, ID, IgnoreTimescale,
                            DebugWorldPositionTarget.transform.position);
    }
    ...
    {
        if (CameraMode == CameraModes.Main) { TargetCamera = Camera.main; }
    ...
    protected virtual void Fade()
    {
        ...
        {
            float newScale = MMTween.Tween(currentTime, _fadeStartedAt, endTime, _initialScale, _currentTargetScale,
                                           _currentCurve);
    ...
    protected virtual void StopFading()
    {
        ...
        _fading = false;
        if (FaderMask.transform.localScale == MaskScale.y * Vector3.one) { DisableFader(); }
    }
        ...
        bool ignoreTimeScale, Vector3 worldPosition)
    {
        ...
        if (TargetCamera == null)
        {
            Debug.LogWarning(this.name
                             + " : You're using a fader round but its TargetCamera hasn't been setup in its inspector. It can't fade.");
            return;
        }
    ...
    public virtual void OnMMEvent(MMFadeEvent fadeEvent)
    {
        ...
        StartFading(FaderMask.transform.localScale.x, _currentTargetScale, fadeEvent.Duration, fadeEvent.Curve,
                    fadeEvent.ID,
