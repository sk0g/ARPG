...
public class MMFader : MonoBehaviour, MMEventListener<MMFadeEvent>, MMEventListener<MMFadeInEvent>,
    MMEventListener<MMFadeOutEvent>, MMEventListener<MMFadeStopEvent>
{
    ...
    protected virtual void StopFading()
    {
        ...
        _fading = false;
        if (_canvasGroup.alpha == InactiveAlpha) { DisableFader(); }
    }
        ...
        bool ignoreTimeScale)
    {
        ...
        if (Time.frameCount == 1) { _frameCountOne = true; }
    }
    ...
    public virtual void OnMMEvent(MMFadeEvent fadeEvent)
    {
        ...
        StartFading(_canvasGroup.alpha, _currentTargetAlpha, fadeEvent.Duration, fadeEvent.Curve, fadeEvent.ID,
                    fadeEvent.IgnoreTimeScale);
    }
    ...
    {
        StartFading(InactiveAlpha, ActiveAlpha, fadeEvent.Duration, fadeEvent.Curve, fadeEvent.ID,
                    fadeEvent.IgnoreTimeScale);
    }
    ...
    {
        StartFading(ActiveAlpha, InactiveAlpha, fadeEvent.Duration, fadeEvent.Curve, fadeEvent.ID,
                    fadeEvent.IgnoreTimeScale);
    }
