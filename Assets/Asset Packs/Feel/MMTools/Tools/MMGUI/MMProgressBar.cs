...
public class MMProgressBar : MMMonoBehaviour
{
    ...
    [FormerlySerializedAs("DelayedBar")] public Transform DelayedBarDecreasing;
    ...
    [FormerlySerializedAs("EndValue")] [Range(0f, 1f)]
    ...
    [MMCondition("SetInitialFillValueOnStart", true)] [Range(0f, 1f)]
    ...
    [FormerlySerializedAs("LerpForegroundBarSpeed")] [MMCondition("LerpForegroundBar", true)]
    ...
    [FormerlySerializedAs("LerpDelayedBarSpeed")] [MMCondition("LerpDecreasingDelayedBar", true)]
    public float LerpDecreasingDelayedBarSpeed = 15f;

    /// the duration each update of the foreground bar should take (only if in fixed duration bar fill mode)
    [FormerlySerializedAs("LerpDelayedBarDuration")] [MMCondition("LerpDecreasingDelayedBar", true)]
    public float LerpDecreasingDelayedBarDuration = 0.2f;

    /// the curve to use when animating the delayed bar fill
    [FormerlySerializedAs("LerpDelayedBarCurve")] [MMCondition("LerpDecreasingDelayedBar", true)]
    ...
    [MMInspectorGroup("Text", true, 20)] public Text PercentageText;
    ...
    [MMInspectorButton("DebugUpdateBar")] public bool DebugUpdateBarButton;
    [MMInspectorButton("DebugSetBar")] public bool DebugSetBarButton;
    [MMInspectorButton("Bump")] public bool TestBumpButton;
    [MMInspectorButton("Plus10Percent")] public bool Plus10PercentButton;
    [MMInspectorButton("Minus10Percent")] public bool Minus10PercentButton;
    ...
    /// the current progress of the bar, ideally read only
    [Range(0f, 1f)] public float BarTarget;

    [Range(0f, 1f)] public float DelayedBarIncreasingProgress;
    [Range(0f, 1f)] public float DelayedBarDecreasingProgress;
    ...
    public virtual void UpdateBar(float currentValue, float minValue, float maxValue)
    {
        ...
        if ((_newPercent != _percentLastTimeBarWasUpdated) && !Bumping) { Bump(); }

        DetermineDeltaTime();
        _lastUpdateTimestamp = _time;

        DetermineDirection();
        if (_direction < 0) { OnBarMovementDecreasingStart?.Invoke(); }
        ...
        if (this.gameObject.activeInHierarchy) { _coroutine = StartCoroutine(UpdateBarsCo()); }
    ...
    protected virtual void OnEnable()
    {
        ...
        if (_foregroundImage != null) { _foregroundImage.color = _initialColor; }
    }
    ...
    public virtual void Initialization()
    {
        ...
        if (DelayedBarDecreasing != null) { _delayedDecreasingImage = DelayedBarDecreasing.GetComponent<Image>(); }

        if (DelayedBarIncreasing != null) { _delayedIncreasingImage = DelayedBarIncreasing.GetComponent<Image>(); }
        ...
        if (SetInitialFillValueOnStart) { SetBar01(InitialFillValue); }
    }
    ...
    protected virtual void UpdateBars()
    {
        ...
        if (_direction < 0)
        {
            newFill = ComputeNewFill(LerpForegroundBar, LerpForegroundBarSpeedDecreasing,
                                     LerpForegroundBarDurationDecreasing, LerpForegroundBarCurveDecreasing, 0f,
                                     _percentLastTimeBarWasUpdated, out t1);
            ...
            if (_time - _lastUpdateTimestamp > DecreasingDelay)
            {
                newFillDelayed = ComputeNewFill(LerpDecreasingDelayedBar, LerpDecreasingDelayedBarSpeed,
                                                LerpDecreasingDelayedBarDuration, LerpDecreasingDelayedBarCurve,
                                                DecreasingDelay, _delayedBarDecreasingProgress, out t2);
        ...
        else // if the value is increasing
        {
            newFill = ComputeNewFill(LerpForegroundBar, LerpForegroundBarSpeedIncreasing,
                                     LerpForegroundBarDurationIncreasing, LerpForegroundBarCurveIncreasing, 0f,
                                     _delayedBarIncreasingProgress, out t1);
            SetBarInternal(newFill, DelayedBarIncreasing, _delayedIncreasingImage, _initialBarSize);
            ...
            if (DelayedBarIncreasing == null)
            {
                newFill = ComputeNewFill(LerpForegroundBar, LerpForegroundBarSpeedIncreasing,
                                         LerpForegroundBarDurationIncreasing, LerpForegroundBarCurveIncreasing, 0f,
                                         _percentLastTimeBarWasUpdated, out t2);
            ...
            else
            {
                ...
                {
                    newFillDelayed = ComputeNewFill(LerpIncreasingDelayedBar, LerpForegroundBarSpeedIncreasing,
                                                    LerpForegroundBarDurationIncreasing,
                                                    LerpForegroundBarCurveIncreasing, IncreasingDelay,
                                                    _delayedBarDecreasingProgress, out t2);
        ...
        {
            _coroutineShouldRun = false;
            if (_direction > 0) { OnBarMovementIncreasingStop?.Invoke(); }
            else { OnBarMovementDecreasingStop?.Invoke(); }
        }
    }

    protected virtual float ComputeNewFill(bool lerpBar, float barSpeed, float barDuration, AnimationCurve barCurve,
        float delay, float lastPercent, out float t)
    {
        float newFill = 0f;
        t = 0f;
        if (lerpBar)
        {
            ...
            float duration = (BarFillMode == BarFillModes.FixedDuration)
                ? barDuration
                : (Mathf.Abs(_newPercent - lastPercent)) / speed;
    ...
    public virtual void Bump()
    {
        ...
        if (BumpOnIncrease && (_direction > 0)) { shouldBump = true; }

        if (BumpOnDecrease && (_direction < 0)) { shouldBump = true; }

        if (BumpScaleOnChange) { shouldBump = true; }

        if (!shouldBump) { return; }

        if (this.gameObject.activeInHierarchy) { StartCoroutine(BumpCoroutine()); }
    ...
    protected virtual IEnumerator BumpCoroutine()
    {
        ...
        if (ChangeColorWhenBumping && (_foregroundImage != null)) { _foregroundImage.color = _initialColor; }
    ...
    {
        if (delay <= 0) { this.gameObject.SetActive(false); }
        else if (this.gameObject.activeInHierarchy) { StartCoroutine(HideBarCo(delay)); }
    }
