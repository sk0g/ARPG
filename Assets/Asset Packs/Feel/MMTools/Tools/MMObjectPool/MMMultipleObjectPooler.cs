...
public class MMMultipleObjectPooler : MMObjectPooler
{
    ...
    public override void FillObjectPool()
    {
        ...
        for (int i = 0; i < Pool.Count; i++) { _randomizedPool.Add(Pool[i]); }
        ...
        case MMPoolingMethods.OriginalOrder:
            ...
            for (int i = 0; i < Pool.Count; i++) { poolSizes[i] = Pool[i].PoolSize; }
        ...
        default:
            ...
            foreach (MMMultipleObjectPoolerObject pooledGameObject in Pool)
            {
                ...
                for (int j = 0; j < Pool[k].PoolSize; j++) { AddOneObjectToThePool(pooledGameObject.GameObjectToPool); }
        ...
        if ((PoolingMethod == MMPoolingMethods.OriginalOrder)
    ...
    public override GameObject GetPooledGameObject()
    {
        ...
        if (pooledGameObject != null) { _lastPooledObjectName = pooledGameObject.name; }
        else { _lastPooledObjectName = ""; }
    ...
    protected virtual GameObject GetPooledGameObjectOriginalOrder()
    {
        ...
        if (_currentIndex >= _pooledGameObjectsOriginalOrder.Count) { ResetCurrentIndex(); }
        ...
        if (_pooledGameObjects[_currentIndex].gameObject.activeInHierarchy)
        {
            GameObject findObject =
                FindInactiveObject(_pooledGameObjects[_currentIndex].gameObject.name, _pooledGameObjects);
            if (findObject != null)
            {
                _currentIndex++;
                return findObject;
    ...
    protected virtual GameObject GetPooledGameObjectPoolSizeBased()
    {
        ...
        if (!PoolObjectEnabled(_pooledGameObjects[randomIndex])) { return null; }
        ...
        if (_pooledGameObjects[randomIndex].gameObject.activeInHierarchy)
        {
            ...
            GameObject pulledObject =
                FindInactiveObject(_pooledGameObjects[randomIndex].gameObject.name, _pooledGameObjects);
            ...
            else
            {
                ...
                if (searchedObject.PoolCanExpand) { return AddOneObjectToThePool(searchedObject.GameObjectToPool); }
                else
                {
    ...
    protected virtual GameObject GetPooledGameObjectRandomBetweenObjects()
    {
        ...
        // if we can't pool the same object twice, we'll loop for a while to try and get another one
        while (!CanPoolSameObjectTwice
               && Pool[randomIndex].GameObjectToPool.name == _lastPooledObjectName
               && overflowCounter < _pooledGameObjects.Count)
        {
    ...
    protected virtual bool PoolObjectEnabled(GameObject testedObject)
    {
        ...
        if (searchedObject != null) { return searchedObject.Enabled; }
        else { return false; }
    }
    ...
    public virtual void EnableObjects(string name, bool newStatus)
    {
        foreach (MMMultipleObjectPoolerObject poolerObject in Pool)
        {
            if (name.Equals(poolerObject.GameObjectToPool.name)) { poolerObject.Enabled = newStatus; }
        }
    }
