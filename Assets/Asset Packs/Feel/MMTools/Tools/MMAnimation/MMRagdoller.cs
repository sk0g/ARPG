...
public class MMRagdoller : MonoBehaviour
{
        ...
        set
        {
            if (value == true)
            {
                ...
                if (CurrentState == RagdollStates.Animated)
                {
                    ...
                    MMAnimatorExtensions.UpdateAnimatorBool(_animator, _getUpFromBackAnimationParameter, false,
                                                            _animatorParameters);
                    MMAnimatorExtensions.UpdateAnimatorBool(_animator, _getUpFromBellyAnimationParameter, false,
                                                            _animatorParameters);
                }
            }
            else
            {
                if (CurrentState == RagdollStates.Ragdolling)
                {
                    ...
                    _ragdolledFeetPosition =
                        0.5f
                        * (_animator.GetBoneTransform(HumanBodyBones.LeftToes).position
                           + _animator.GetBoneTransform(HumanBodyBones.RightToes).position);
                    ...
                    {
                        MMAnimatorExtensions.UpdateAnimatorBool(_animator, _getUpFromBackAnimationParameter, true,
                                                                _animatorParameters);
                    }
                    else
                    {
                        MMAnimatorExtensions.UpdateAnimatorBool(_animator, _getUpFromBellyAnimationParameter, true,
                                                                _animatorParameters);
                    }
                }
            }
    ...
    protected virtual void Initialization()
    {
        ...
        if (CurrentState == RagdollStates.Animated) { SetIsKinematic(true); }
        else { SetIsKinematic(false); }
    ...
    public virtual void ForceRigidbodiesToSleep()
    {
        foreach (Component rigidbody in _rigidbodies)
        {
            if (rigidbody.transform != this.transform) { (rigidbody as Rigidbody).Sleep(); }
        }
    }
    ...
    protected virtual void HandleBlending()
    {
        if (CurrentState == RagdollStates.Blending)
        {
            if (Time.time <= _ragdollingEndTimestamp + _mecanimToGetUpTransitionTime)
            {
                ...
                transform.rotation *=
                    Quaternion.FromToRotation(animatedDirection.normalized, ragdollingDirection.normalized);
            }

            float ragdollBlendAmount =
                1.0f
                - (Time.time - _ragdollingEndTimestamp - _mecanimToGetUpTransitionTime) / RagdollToMecanimBlendDuration;
            ...
            foreach (RagdollBodyPart bodypart in _bodyparts)
            {
                if (bodypart.BodyPartTransform != transform)
                {
                    ...
                    {
                        bodypart.BodyPartTransform.position =
                            Vector3.Lerp(bodypart.BodyPartTransform.position, bodypart.StoredPosition,
                                         ragdollBlendAmount);
                    }

                    bodypart.BodyPartTransform.rotation =
                        Quaternion.Slerp(bodypart.BodyPartTransform.rotation, bodypart.StoredRotation,
                                         ragdollBlendAmount);
                }
            }
