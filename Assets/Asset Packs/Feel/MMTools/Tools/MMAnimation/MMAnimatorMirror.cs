...
public class MMAnimatorMirror : MonoBehaviour
{
    ...
    {
        if (TargetAnimator == null) { TargetAnimator = this.gameObject.GetComponent<Animator>(); }

        if ((TargetAnimator == null) || (SourceAnimator == null)) { return; }
        ...
        for (int i = 0; i < numberOfParameters; i++) { _sourceParameters[i] = SourceAnimator.GetParameter(i); }
        ...
        for (int i = 0; i < numberOfParameters; i++) { _targetParameters[i] = TargetAnimator.GetParameter(i); }
    ...
    protected virtual void Mirror()
    {
        ...
        foreach (MMAnimatorMirrorBind bind in _updateParameters)
        {
            ...
            case AnimatorControllerParameterType.Trigger:
                if (SourceAnimator.GetBool(bind.ParameterHash)) { TargetAnimator.SetTrigger(bind.ParameterHash); }
