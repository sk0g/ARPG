...
public class MMPathMovementEditor : Editor
{
    ...
    {
        get { return (MMPathMovement) target; }
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        else { Editor.DrawPropertiesExcluding(serializedObject, new string[] {"Acceleration"}); }
    ...
    protected virtual void OnSceneGUI()
    {
        ...
        for (int i = 0; i < t.PathElements.Count; i++)
        {
            ...
            Handles.Label(
                t.GetOriginalTransformPosition()
                + t.PathElements[i].PathElementPosition
                + (Vector3.down * 0.4f)
                + (Vector3.right * 0.4f), "" + i, style);

            // draws a movable handle
            Vector3 newPoint = Handles.FreeMoveHandle(oldPoint, Quaternion.identity, .5f, new Vector3(.25f, .25f, .25f),
