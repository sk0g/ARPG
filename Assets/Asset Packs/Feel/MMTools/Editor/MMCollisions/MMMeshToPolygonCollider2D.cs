...
public class MMMeshToPolygonCollider2D : MonoBehaviour
{
    ...
    private static void GeneratePolygonCollider2D(MeshFilter meshFilter)
    {
        ...
        if (!ValidateMesh(meshFilter)) { return; }
    ...
    private static Vector2[] VectorsToPoints(Vector3[] vectors)
    {
        ...
        for (int i = 0; i < vectors.Length; i++) { newColliderVertices.Add(new Vector2(vectors[i].x, vectors[i].y)); }
    ...
    private static Vector3[] MeshFilterToVectors(MeshFilter meshFilter)
    {
        ...
        List<MMGeometry.MMEdge> boundaryPath =
            MMGeometry.GetEdges(meshFilter.sharedMesh.triangles).FindBoundary().SortEdges();

        Vector3[] vectors = new Vector3[boundaryPath.Count];
        for (int i = 0; i < boundaryPath.Count; i++) { vectors[i] = vertices[boundaryPath[i].Vertice1]; }
    ...
    private static PolygonCollider2D InitializePolygonCollider2D(MeshFilter meshFilter)
    {
        ...
        if (polygonCollider2D == null) { polygonCollider2D = meshFilter.gameObject.AddComponent<PolygonCollider2D>(); }
    ...
    public static void GeneratePolygonCollider2DMenu()
    {
        ...
        foreach (MeshFilter meshFilter in meshFilters) { GeneratePolygonCollider2D(meshFilter); }
    }
}
}