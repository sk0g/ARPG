...
public class MMRadioSignalGeneratorEditor : MMRadioSignalEditor
{
    ...
    {
        DrawPropertiesExcluding(serializedObject, "AnimatedPreview", "SignalList", "GlobalMultiplier", "CurrentLevel",
                                "Clamp", "Clamps");
    ...
    protected virtual void DrawRawSpectrum()
    {
        ...
        if (_animatedPreview.boolValue)
        {
            ...
            zeroX = maxX
                    - MMMaths.Remap(_normalizedTime, 0f, 1f, _spectrumBoxPosition.x + _externalMargin,
        ...
        if (_duration.floatValue > 0f)
        {
            ...
            {
                boxSpectrumValue =
                    MMMaths.Remap(_radioSignal.GraphValue(_normalizedTime), 0f, 1f, 0f, _spectrumBoxSize.y);
            }

            _rect.y = _spectrumBoxBottomY - boxSpectrumValue - _externalMargin / 4;
        }
        else { _rect.y = _spectrumBoxBottomY; }
        ...
        for (int i = 1; i < _spectrumPointsCount; i++)
        {
            float xPosition = _spectrumBoxPosition.x
                              + _externalMargin
                              + MMMaths.Remap(i, 0, _spectrumPointsCount, 0f, _spectrumBoxSize.x - _externalMargin * 2);
