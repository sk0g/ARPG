...
public class MMPropertyPickerDrawer : PropertyDrawer
{
    ...
    public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
    {
        ...
        {
            _numberOfLines = 3;
            if (_selectedComponentIndex != 0) { _numberOfLines = 4; }
        }

        if (_targetIsScriptableObject) { _numberOfLines = 4; }
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        Rect targetObjectRect =
            new Rect(position.x, position.y + (_lineHeight + _lineMargin), position.width, _lineHeight);
        Rect targetComponentRect = new Rect(position.x, position.y + (_lineHeight + _lineMargin) * 2, position.width,
                                            _lineHeight);
        Rect targetPropertyRect = new Rect(position.x, position.y + (_lineHeight + _lineMargin) * 3, position.width,
                                           _lineHeight);
        ...
        EditorGUI.PropertyField(targetObjectRect, property.FindPropertyRelative("TargetObject"),
                                new GUIContent("Target Object"), true);
        if (EditorGUI.EndChangeCheck())
        {
            ...
            if (_targetIsScriptableObject) { FillPropertyList(property); }
        }

        // displays a label for scriptable objects
        if (_targetIsScriptableObject) { EditorGUI.LabelField(targetComponentRect, "Type", "Scriptable Object"); }
        ...
        if ((_componentNames != null) && (_componentNames.Length > 0))
        {
            ...
            _selectedComponentIndex =
                EditorGUI.Popup(targetComponentRect, "Component", _selectedComponentIndex, _componentNames);
            if (EditorGUI.EndChangeCheck())
            {
                SetTargetComponent(property);
                _selectedPropertyIndex = 0;
        ...
        if (((_selectedComponentIndex != 0) || _targetIsScriptableObject)
            && (_propertiesNames != null)
            && (_propertiesNames.Length > 0))
        {
            EditorGUI.BeginChangeCheck();
            _selectedPropertyIndex =
                EditorGUI.Popup(targetPropertyRect, "Property", _selectedPropertyIndex, _propertiesNames);
            if (EditorGUI.EndChangeCheck()) { SetTargetProperty(property); }
        }
    ...
    {
        Rect levelLabelRect = new Rect(position.x, position.y + (_lineHeight + _lineMargin) * (_numberOfLines - 1),
                                       position.width, _lineHeight);
        Rect levelValueRect = new Rect(position.x - 15 + EditorGUIUtility.labelWidth + 4,
                                       position.y + (_lineHeight + _lineMargin) * (_numberOfLines - 1), position.width,
                                       _lineHeight);
        ...
        if (negative) { EditorGUI.DrawRect(levelProgressFront, negativeColor); }
        else { EditorGUI.DrawRect(levelProgressFront, frontColor); }
    }
    ...
    protected virtual void FillPropertyList(SerializedProperty property)
    {
        ...
        if ((property.FindPropertyRelative("TargetComponent").objectReferenceValue == null)
            && !_targetIsScriptableObject) { return; }
        ...
        {
            // Find all fields
            var fieldsList = property.FindPropertyRelative("TargetComponent")
                                     .objectReferenceValue.GetType()
                                     .GetFields(BindingFlags.Public | BindingFlags.Instance)
                                     .Where(field =>
                                                (AuthorizedType(_authorizedTypes, field.FieldType))
                                     )
                                     .OrderBy(prop => prop.FieldType.Name)
                                     .ToList();
            ...
            var propertiesList = property.FindPropertyRelative("TargetComponent")
                                         .objectReferenceValue.GetType()
                                         .GetProperties(BindingFlags.Public | BindingFlags.Instance)
                                         .Where(prop =>
                                                    (AuthorizedType(_authorizedTypes, prop.PropertyType))
                                         )
                                         .OrderBy(prop => prop.PropertyType.Name)
                                         .ToList();
        ...
        else
        {
            ...
            // finds all fields
            var fieldsList = property.FindPropertyRelative("TargetObject")
                                     .objectReferenceValue.GetType()
                                     .GetFields(BindingFlags.Public | BindingFlags.Instance)
                                     .Where(field =>
                                                (AuthorizedType(_authorizedTypes, field.FieldType))
            ...
            // finds all properties
            var propertiesList = property.FindPropertyRelative("TargetObject")
                                         .objectReferenceValue.GetType()
                                         .GetProperties(BindingFlags.Public | BindingFlags.Instance)
                                         .Where(prop =>
                                                    (AuthorizedType(_authorizedTypes, prop.PropertyType))
    ...
    protected virtual void SetTargetComponent(SerializedProperty property)
    {
        ...
        {
            property.serializedObject.Update();
            property.FindPropertyRelative("TargetScriptableObject").objectReferenceValue =
                property.FindPropertyRelative("TargetObject").objectReferenceValue as ScriptableObject;
        ...
        {
            property.serializedObject.Update();
            property.FindPropertyRelative("TargetComponent").objectReferenceValue =
                _componentList[_selectedComponentIndex];
    ...
    protected virtual void GetComponentIndex(SerializedProperty property)
    {
        ...
        if (!found) { _selectedComponentIndex = 0; }
    }
    ...
    protected virtual void GetPropertyIndex(SerializedProperty property)
    {
        ...
        ScriptableObject targetScriptable =
            property.FindPropertyRelative("TargetScriptableObject").objectReferenceValue as ScriptableObject;

        if ((targetComponent == null) && (targetScriptable == null)) { return; }
        ...
        if (!found) { _selectedPropertyIndex = 0; }
    }

    protected virtual Type GetPropertyType(SerializedProperty property)
    {
        if (_selectedPropertyIndex == 0) { return null; }

        MMProperty tempProperty;

        tempProperty = MMProperty.FindProperty(_propertiesList[_selectedPropertyIndex],
                                               property.FindPropertyRelative("TargetComponent")
                                                       .objectReferenceValue as Component, null,
                                               property.FindPropertyRelative("TargetObject").objectReferenceValue as
                                                   ScriptableObject);

        if (tempProperty != null) { return tempProperty.PropertyType; }
        else { return null; }
    }
}
}