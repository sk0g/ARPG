...
public class ReorderableList
{
    ...
    {
        if (HasList) { return list.isExpanded ? headerHeight + GetElementsHeight() + footerHeight : headerHeight; }
        ...
        else
        {
            ...
            if (list.isExpanded)
            {
                ...
                if (list.arraySize > 0)
                {
                    ...
                    if (!dragging) { UpdateElementRects(elementBackgroundRect, evt); }
                ...
                else { DrawEmpty(elementBackgroundRect, "List is Empty", Style.boxBackground, Style.verticalLabel); }
    ...
    public SerializedProperty AddItem<T>(T item) where T : Object
    {
        ...
        if (property != null) { property.objectReferenceValue = item; }
    ...
    public SerializedProperty AddItem()
    {
        ...
        else { throw new InvalidListException(); }
    }
    ...
    public void Remove(int[] selection)
    {
        ...
        while (--i > -1) { RemoveItem(selection[i]); }
    }
    ...
    public void RemoveItem(int index)
    {
        if (HasList && index >= 0 && index < list.arraySize)
        {
            ...
            if (list.arraySize > 0) { selection.Select(Mathf.Max(0, index - 1)); }
    ...
    {
        if (HasList && index >= 0 && index < list.arraySize) { return list.GetArrayElementAtIndex(index); }
        else { return null; }
    }
    ...
    public int IndexOf(SerializedProperty element)
    {
        if (element != null)
        {
            ...
            while (--i > -1)
            {
                if (SerializedProperty.EqualContents(element, list.GetArrayElementAtIndex(i))) { return i; }
            }
        }
    ...
    {
        if (getElementsHeightCallback != null) { return getElementsHeightCallback(this); }
        ...
        for (i = 0; i < len; i++) { totalHeight += GetElementHeight(list.GetArrayElementAtIndex(i)) + spacing; }
    ...
    {
        if (slideEasing <= 0) { return desiredRect; }
        else
        {
    ...
    */
    ...
    private void DrawHeader(Rect rect, GUIContent label)
    {
        ...
        if (drawHeaderCallback != null) { drawHeaderCallback(titleRect, label); }
        else if (expandable)
        {
            titleRect.xMin += 10;
            ...
            if (EditorGUI.EndChangeCheck()) { list.isExpanded = isExpanded; }
        }
        else { GUI.Label(titleRect, label, EditorStyles.label); }
    ...
    {
        if (!list.isExpanded && expand) { list.isExpanded = true; }

        for (int i = 0; i < list.arraySize; i++) { list.GetArrayElementAtIndex(i).isExpanded = expand; }
    }
    ...
    private void UpdateElementRects(Rect rect, Event evt)
    {
        ...
        if (len != elementRects.Length) { System.Array.Resize(ref elementRects, len); }
    ...
    private void DrawElement(SerializedProperty element, Rect rect, bool selected, bool focused)
    {
        ...
        if (drawElementCallback != null) { drawElementCallback(renderRect, element, label, selected, focused); }
        ...
        case EventType.ContextClick:
            ...
            if (rect.Contains(evt.mousePosition)) { HandleContextClick(evt, element); }
    ...
    private GUIContent GetElementLabel(SerializedProperty element)
    {
        ...
        if (getElementNameCallback != null) { name = getElementNameCallback(element); }
        else { name = GetElementName(element, elementNameProperty, elementNameOverride); }
    ...
    private static string GetElementName(SerializedProperty element, string nameProperty, string nameOverride)
    {
        ...
        else if (string.IsNullOrEmpty(nameProperty)) { return null; }
    ...
    private static string GetLayerMaskName(int mask)
    {
        ...
        else if (mask < 0) { return "Everything"; }
    ...
    private void DrawFooter(Rect rect)
    {
        ...
        if (GUI.Button(addRect, onAddDropdownCallback != null ? Style.iconToolbarPlusMore : Style.iconToolbarPlus,
                       Style.preButton))
        {
            if (onAddDropdownCallback != null) { onAddDropdownCallback(addRect, this); }
            else if (onAddCallback != null) { onAddCallback(this); }
            else { AddItem(); }
        }

        EditorGUI.EndDisabledGroup();

        EditorGUI.BeginDisabledGroup(!CanSelect(selection)
                                     || !canRemove
    ...
    private void HandleContextClick(Event evt, SerializedProperty element)
    {
        ...
        {
            menu.AddItem(new GUIContent("Revert " + GetElementLabel(element).text + " to Prefab"), false,
            ...
            if (GUI.enabled && rect.Contains(evt.mousePosition))
            {
                ...
                foreach (Object object1 in objectReferences)
                {
                    ...
                    if (object2 != null)
                    {
                        ...
                        {
                            if (onAppendDragDropCallback != null) { onAppendDragDropCallback(object2, this); }
                            else { AppendDragAndDropValue(object2); }

                            acceptDrag = true;
                            DragAndDrop.activeControlID = 0;
                        }
                        else { DragAndDrop.activeControlID = dragDropControlID; }
                    }
                }
        ...
        case EventType.MouseDown:
            ...
            if (rect.Contains(evt.mousePosition) && IsSelectionButton(evt))
            {
                ...
                if (CanSelect(index))
                {
                    DoSelection(
                        index,
                        GUIUtility.keyboardControl == 0 || GUIUtility.keyboardControl == controlID || evt.button == 2,
                        evt);
                }
    ...
    private void DoSelection(int index, bool setKeyboardControl, Event evt)
    {
        ...
        if (multipleSelection) { selection.AppendWithAction(pressIndex = index, evt); }
        else { selection.Select(pressIndex = index); }
    ...
    private DragElement[] GetDragList(float dragPosition)
    {
        ...
        if (dragList == null) { dragList = new DragElement[len]; }
        else if (dragList.Length != len) { System.Array.Resize(ref dragList, len); }
        ...
        {
            if (b.selected) { return a.selected ? a.startIndex.CompareTo(b.startIndex) : 1; }
            else if (a.selected) { return b.selected ? b.startIndex.CompareTo(a.startIndex) : -1; }
    ...
    private void ReorderDraggedElements(DragElement[] dragList)
    {
        ...
        for (int i = 0; i < dragList.Length; i++) { dragList[i].RecordState(); }
        ...
        for (int i = 0; i < dragList.Length; i++) { dragList[i].RestoreState(list.GetArrayElementAtIndex(i)); }
    }
    ...
    private int GetSelectionIndex(Vector2 position)
    {
        ...
        for (i = 0; i < len; i++)
        {
            ...
            if (rect.Contains(position)
                || (i == 0 && position.y <= rect.yMin)
                || (i == len - 1 && position.y >= rect.yMax)) { return i; }
        }
    ...
    struct DragElement
    {
        ...
        {
            if (direction < 0 && index < startIndex) { return desiredRect.yMin < value.center.y; }
            else if (direction > 0 && index > startIndex) { return desiredRect.yMax > value.center.y; }
    ...
    class SlideGroup
    {
        ...
        public Rect GetRect(int id, Rect r, float easing)
        {
            ...
            else
            {
                ...
                if (rect.y != r.y)
                {
                    ...
                    if (absDelta > (rect.height * 2)) { r.y = delta > 0 ? r.y - rect.height : r.y + rect.height; }
        ...
        {
            if (animIDs.ContainsKey(id)) { animIDs[id] = rect; }
            else { animIDs.Add(id, rect); }
    ...
    class ListSelection : IEnumerable<int>
    {
        ...
        public void AppendWithAction(int index, Event evt)
        {
            ...
            else if (!Contains(index)) { Select(index); }
        }
        ...
        internal bool CanRevert(SerializedProperty list)
        {
            if (list.serializedObject.targetObjects.Length == 1)
            {
                ...
                {
                    if (list.GetArrayElementAtIndex(this[i]).isInstantiatedPrefab) { return true; }
                }
            }
        ...
        internal void Delete(SerializedProperty list)
        {
            ...
            while (--i > -1) { list.GetArrayElementAtIndex(this[i]).DeleteCommand(); }
        ...
        private void AppendRange(int from, int to)
        {
            ...
            if (dir != 0)
            {
                for (int i = from; i != to; i += dir) { Append(i); }
            }
