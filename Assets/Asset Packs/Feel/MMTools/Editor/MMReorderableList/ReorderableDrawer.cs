...
public class ReorderableDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (list != null) { list.DoList(EditorGUI.IndentedRect(position), label); }
        else { GUI.Label(position, "Array must extend from ReorderableArray", EditorStyles.label); }
    }
    ...
    public static ReorderableList GetList(SerializedProperty property, MMReorderableAttributeAttribute attrib, int id)
    {
        ...
        if (array != null && array.isArray)
        {
            if (!lists.TryGetValue(id, out list))
            {
                ...
                else { list = new ReorderableList(array, true, true, true); }

                lists.Add(id, list);
            }
            else { list.List = array; }
        }
