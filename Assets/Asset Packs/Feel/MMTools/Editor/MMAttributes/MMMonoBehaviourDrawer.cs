...
public class MMMonoBehaviourDrawer : UnityEditor.Editor
{
    ...
    protected virtual void OnEnable()
    {
        ...
        MMHiddenPropertiesAttribute[] hiddenProperties =
    ...
    protected virtual void Initialization()
    {
        ...
        {
            MMInspectorGroupAttribute group =
            ...
            if (!GroupData.TryGetValue(group.GroupName, out groupData))
            {
                bool groupIsOpen =
                    EditorPrefs.GetBool(
                        string.Format($"{group.GroupName}{fieldInfoList[i].Name}{target.GetInstanceID()}"), false);
        ...
        {
            do { FillPropertiesList(iterator); } while (iterator.NextVisible(false));
        }
    ...
    protected virtual void DrawGroup(MMInspectorGroupData groupData)
    {
        ...
        groupData.GroupIsOpen = EditorGUILayout.Foldout(groupData.GroupIsOpen, groupData.GroupAttribute.GroupName, true,
                                                        MMMonoBehaviourDrawerStyle.GroupStyle);
        ...
        void DrawChild(int i)
        {
            ...
            EditorGUILayout.PropertyField(groupData.PropertiesList[i],
                                          new GUIContent(
                                              ObjectNames.NicifyVariableName(groupData.PropertiesList[i].name)), true);
        }
    }
