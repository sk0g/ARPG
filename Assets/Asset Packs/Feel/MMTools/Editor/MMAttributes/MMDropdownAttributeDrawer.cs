...
public class MMDropdownAttributeDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        if (_dropdownAttribute == null)
        {
            ...
            if (_dropdownAttribute.DropdownValues == null || _dropdownAttribute.DropdownValues.Length == 0) { return; }
            ...
            for (var i = 0; i < _dropdownValues.Length; i++)
            {
                ...
                if ((_propertyType == typeof(float))
                    && Mathf.Approximately(property.floatValue, Convert.ToSingle(_dropdownValues[i])))
                {
                    _selectedDropdownValueIndex = i;
                    found = true;
                    break;
            ...
            if (!found) { _selectedDropdownValueIndex = 0; }
        }
        ...
        _selectedDropdownValueIndex =
            EditorGUI.Popup(position, label.text, _selectedDropdownValueIndex, _dropdownValues);
