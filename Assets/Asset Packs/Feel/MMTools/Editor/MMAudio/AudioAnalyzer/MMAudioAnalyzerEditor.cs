...
public class MMAudioAnalyzerEditor : Editor
{
    ...
    {
        if (!Application.isPlaying) { _numberOfBands = serializedObject.FindProperty("NumberOfBands").intValue; }
    }
    ...
    protected virtual void DrawBandTable()
    {
        ...
        if (!Active)
        {
            if (_bandValuesFoldout) { GUILayout.Label("Values are only displayed when the game is running."); }

            return;
        }

        if (BandPeaks.arraySize == 0) { return; }
        ...
        if (_bandValuesFoldout)
        {
            ...
            for (int i = 0; i < _numberOfBands; i++)
            {
                ...
                GUILayout.Label(NormalizedBandLevels.GetArrayElementAtIndex(i).floatValue.ToString(),
                                GUILayout.Width(w4));
                GUILayout.Label(NormalizedBufferedBandLevels.GetArrayElementAtIndex(i).floatValue.ToString(),
                                GUILayout.Width(w5));
                GUILayout.FlexibleSpace();
                GUILayout.EndHorizontal();
            }
    ...
    {
        if ((Beats == null) || (target as MMAudioAnalyzer).Beats == null) { return; }
        ...
        int boxesPerLine =
            (int) Mathf.Round((_inspectorWidth - margin - 3 * _externalMargin) / (beatsBoxSquareSize + margin));
    ...
    protected virtual void DrawBandVisualization()
    {
        ...
        for (int i = 0; i < _numberOfBands; i++)
        {
            if (Active)
            {
                ...
                _barColor = (Time.time - LastPeaksAt.GetArrayElementAtIndex(i).floatValue < _peakShowDuration / 3f)
                    ? _activePeakColor
                    : _bufferedBarColor;
        ...
        for (int i = 0; i < _numberOfBands; i++)
        {
            if (Active)
            {
                ...
                _columnHeight = MMMaths.Remap(bandLevel, 0f, 1f, 0f, _maxColumnHeight);
                _barColor = (Time.time - LastPeaksAt.GetArrayElementAtIndex(i).floatValue < _peakShowDuration)
                    ? _peakColor
                    : _normalBarColor;
            }
            else
    ...
    protected virtual void DrawRawSpectrum()
    {
        ...
        if (Active)
        {
            ...
            {
                float xPosition = _spectrumBoxPosition.x
                                  + _externalMargin
                                  + MMMaths.Remap(i, 0, RawSpectrum.arraySize, 0f,
        ...
        else
        {
            ...
            for (int i = 1; i < points - 1; i++)
            {
                float xPosition = _spectrumBoxPosition.x
                                  + _externalMargin
                                  + MMMaths.Remap(i, 0, points, 0f, _spectrumBoxSize.x - _externalMargin * 2);
                float yPosition = _spectrumBoxPosition.y + _spectrumBoxSize.y / 2;
    ...
    {
        if ((target as MMAudioAnalyzer).Mode != MMAudioAnalyzer.Modes.AudioSource) { return; }
        ...
        if (!PeaksPasted.boolValue)
        {
            if ((PeaksSaver.Peaks == null) || (PeaksSaver.Peaks.Length == 0))
            {
                EditorGUILayout.HelpBox(
                    "You haven't preprocessed peaks for this track yet. It's recommended to do so, by pressing play, "
                    + "then the Find Peaks button below. Then, exit play, and press the 'Paste Peaks' button.",
                    MessageType.Warning);
                if (GUILayout.Button("Find Peaks")) { (target as MMAudioAnalyzer).FindPeaks(); }
            }
            else
            {
                EditorGUILayout.HelpBox(
                    "Exit Play Mode first, then paste your saved peaks using the 'Paste Peaks' button below.",
                    MessageType.Warning);
                if (!Application.isPlaying)
                {
                    if (GUILayout.Button("Paste Peaks")) { (target as MMAudioAnalyzer).PastePeaks(); }
                }
            }
        }

        if (GUILayout.Button("Clear Peaks")) { (target as MMAudioAnalyzer).ClearPeaks(); }
    }
}
#endif
}