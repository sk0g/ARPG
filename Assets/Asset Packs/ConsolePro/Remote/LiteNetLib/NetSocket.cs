...
internal sealed class NetSocket
{
    ...
    private void ReceiveLogic(object state)
    {
        ...
        EndPoint bufferEndPoint =
            new IPEndPoint(socket.AddressFamily == AddressFamily.InterNetwork ? IPAddress.Any : IPAddress.IPv6Any, 0);
        ...
        {
            //wait for data
            if (!socket.Poll(SocketReceivePollTime, SelectMode.SelectRead)) { continue; }
            ...
            catch (SocketException ex)
            {
                if (ex.SocketErrorCode == SocketError.ConnectionReset || ex.SocketErrorCode == SocketError.MessageSize)
                {
                    ...
                    //Just UDP
                    NetUtils.DebugWrite(ConsoleColor.DarkRed, "[R] Ingored error: {0} - {1}", (int) ex.SocketErrorCode,
                                        ex.ToString());
                    continue;
                }
    ...
    public bool Bind(int port, bool reuseAddress)
    {
        ...
        try { _udpSocketv4.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.Broadcast, 1); }
        catch (SocketException e) { NetUtils.DebugWriteError("Broadcast error: {0}", e.ToString()); }

        if (!BindSocket(_udpSocketv4, new IPEndPoint(IPAddress.Any, port))) { return false; }
    ...
    private bool BindSocket(Socket socket, IPEndPoint ep)
    {
        ...
        catch (SocketException ex)
        {
            ...
            if (ex.SocketErrorCode == SocketError.AddressFamilyNotSupported) { return true; }
    ...
    public int SendTo(byte[] data, int offset, int size, NetEndPoint remoteEndPoint, ref int errorCode)
    {
        ...
        {
            if (ex.SocketErrorCode != SocketError.MessageSize) { NetUtils.DebugWriteError("[S]" + ex); }
    ...
    public void Close()
    {
        ...
        //Close IPv4
        if (Thread.CurrentThread != _threadv4) { _threadv4.Join(); }
        ...
        //Close IPv6
        if (Thread.CurrentThread != _threadv6) { _threadv6.Join(); }
