...
internal sealed class ReliableChannel
{
    ...
    public ReliableChannel(NetPeer peer, bool ordered, int windowSize)
    {
        ...
        for (int i = 0; i < _pendingPackets.Length; i++) { _pendingPackets[i] = new PendingPacket(); }
    ...
    public void ProcessAck(NetPacket packet)
    {
        ...
        for (int i = 0; i < _windowSize; i++)
        {
            ...
            else { NetUtils.DebugWrite("[PA]Removing reliableInOrder ack: {0} - false", ackSequence); }
        }
    ...
    {
        lock (_outgoingPackets) { _outgoingPackets.Enqueue(packet); }
    }
    ...
    private void ProcessQueuedPackets()
    {
        ...
        while (_outgoingPackets.Count > 0)
        {
            ...
            {
                NetPacket packet;
                lock (_outgoingPackets) { packet = _outgoingPackets.Dequeue(); }
    ...
    public void SendAcks()
    {
        ...
        do
        {
            if (_outgoingAcks[currentAckIndex]) { data[currentByte] |= (byte) (1 << currentBit); }
    ...
    public void ProcessPacket(NetPacket packet)
    {
        ...
        //holded packet
        if (_ordered) { _receivedPackets[packet.Sequence % _windowSize] = packet; }
        else
        {
