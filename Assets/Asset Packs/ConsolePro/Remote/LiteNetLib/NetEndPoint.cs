...
public sealed class NetEndPoint
{
    ...
    {
        if (!(obj is NetEndPoint)) { return false; }
    ...
    public NetEndPoint(string hostStr, int port)
    {
        ...
        if (!IPAddress.TryParse(hostStr, out ipAddress))
        {
            ...
            {
                if (hostStr == "localhost") { ipAddress = IPAddress.IPv6Loopback; }
                else { ipAddress = ResolveAddress(hostStr, AddressFamily.InterNetworkV6); }
            }

            if (ipAddress == null) { ipAddress = ResolveAddress(hostStr, AddressFamily.InterNetwork); }
        }

        if (ipAddress == null) { throw new Exception("Invalid address: " + hostStr); }
