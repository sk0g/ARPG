...
public sealed class NetPeer
{
    ...
    internal bool ProcessConnectAccept(NetPacket packet)
    {
        ...
        //check connection id
        if (BitConverter.ToInt64(packet.RawData, 1) != _connectId) { return false; }
    ...
    private void SendPacket(NetPacket packet)
    {
        ...
        case PacketProperty.MtuCheck:
            ...
            if (!_peerListener.SendRawAndRecycle(packet, _remoteEndPoint)) { _finishMtu = true; }
    ...
    private void UpdateRoundTripTime(int roundTripTime)
    {
        ...
        if (_avgRtt < _peerListener.GetStartRtt(_currentFlowMode - 1))
        {
            ...
            if (_goodRttCount > NetConstants.FlowIncreaseThreshold)
            {
                ...
                NetUtils.DebugWrite("[PA]Increased flow speed, RTT: {0}, PPS: {1}", _avgRtt,
                                    _peerListener.GetPacketsPerSecond(_currentFlowMode));
            }
        }
        else if (_avgRtt > _peerListener.GetStartRtt(_currentFlowMode))
        {
            ...
            {
                _currentFlowMode++;
                NetUtils.DebugWrite("[PA]Decreased flow speed, RTT: {0}, PPS: {1}", _avgRtt,
                                    _peerListener.GetPacketsPerSecond(_currentFlowMode));
            }
        }
    ...
    internal void AddIncomingPacket(NetPacket p)
    {
        if (p.IsFragmented)
        {
            NetUtils.DebugWrite("Fragment. Id: {0}, Part: {1}, Total: {2}", p.FragmentId, p.FragmentPart,
                                p.FragmentsTotal);
            ...
            //Check for finish
            if (incomingFragments.ReceivedCount != fragments.Length) { return; }
    ...
    {
        if (packet.Size == 1 || packet.RawData[1] >= NetConstants.PossibleMtu.Length)
            return;

        //MTU auto increase
        if (packet.Property == PacketProperty.MtuCheck)
        {
            if (packet.Size != NetConstants.PossibleMtu[packet.RawData[1]]) { return; }
        ...
        else if (packet.RawData[1] > _mtuIdx) //MtuOk
        {
            ...
            //if maxed - finish.
            if (_mtuIdx == NetConstants.PossibleMtu.Length - 1) { _finishMtu = true; }
    ...
    internal void ProcessPacket(NetPacket packet)
    {
        ...
        case PacketProperty.ConnectRequest:
            ...
            if (newId > _connectId) { _connectId = newId; }
    ...
    private void SendQueuedPackets(int currentMaxSend)
    {
        ...
        while (currentSended < currentMaxSend)
        {
            ...
            if (_reliableOrderedChannel.SendNextPacket()
                ...
                || _simpleChannel.SendNextPacket()) { currentSended++; }
            else
            {
                //no outgoing packets
                break;
    ...
    public void Flush()
    {
        lock (_flushLock) { SendQueuedPackets(int.MaxValue); }
    }
    ...
    internal void Update(int deltaTime)
    {
        ...
        if (maxSendPacketsCount > 0)
        {
            ...
            currentMaxSend = Math.Min(availableSendPacketsCount,
                                      (maxSendPacketsCount * deltaTime) / NetConstants.FlowUpdateTime);
        }
        else { currentMaxSend = int.MaxValue; }
        ...
        if (!_finishMtu)
        {
            ...
            if (_mtuCheckTimer >= MtuCheckDelay)
            {
                _mtuCheckTimer = 0;
                _mtuCheckAttempts++;
                if (_mtuCheckAttempts >= MaxMtuCheckAttempts) { _finishMtu = true; }
                else
                {
                    lock (_mtuMutex)
                    {
        ...
        //Pending send
        lock (_flushLock) { SendQueuedPackets(currentMaxSend); }
    }
