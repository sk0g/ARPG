...
public sealed class NetSerializer
{
    ...
    private sealed class StructInfo
    {
        ...
        {
            Reference = obj;
            for (int i = 0; i < MembersCount; i++) { WriteDelegate[i](writer); }
        }

        public void Read(NetDataReader reader)
        {
            for (int i = 0; i < MembersCount; i++) { ReadDelegate[i](reader); }
        }
    }
    ...
    {
        var t = typeof(T);
        if (_registeredCustomTypes.ContainsKey(t)) { return false; }
    ...
    {
        var t = typeof(T);
        if (BasicTypes.Contains(t) || _registeredCustomTypes.ContainsKey(t)) { return false; }
    ...
    private StructInfo RegisterInternal<T>() where T : class
    {
        ...
        StructInfo info;
        if (_cache.TryGetValue(nameHash, out info)) { return info; }
...
#endif
        if (props == null || propsCount < 0) { throw new ArgumentException("Type does not contain acceptable fields"); }
        ...
        for (int i = 0; i < props.Length; i++)
        {
            ...
            if (isEnum)
            {
                ...
                if (underlyingType == typeof(byte))
                {
                    ...
                    info.WriteDelegate[i] = writer => { writer.Put((byte) property.GetValue(info.Reference, null)); };
                }
                else if (underlyingType == typeof(int))
                {
                    ...
                    info.WriteDelegate[i] = writer => { writer.Put((int) property.GetValue(info.Reference, null)); };
                }
                else { throw new Exception("Not supported enum underlying type: " + underlyingType.Name); }
            }
            ...
            else if (propertyType == typeof(string[]))
            {
                ...
                var getDelegate = ExtractGetDelegate<T, string[]>(getMethod);
                info.ReadDelegate[i] = reader =>
                    setDelegate((T) info.Reference, reader.GetStringArray(MaxStringLenght));
            ...
            else
            {
                ...
                else { throw new Exception("Unknown property type: " + propertyType.Name); }
            }
        }
    ...
    {
        while (reader.AvailableBytes > 0) { ReadPacket(reader); }
    }
    ...
    {
        while (reader.AvailableBytes > 0) { ReadPacket(reader, userData); }
    }
    ...
    private StructInfo ReadInfo(NetDataReader reader)
    {
        ...
        StructInfo info;
        if (!_cache.TryGetValue(hash, out info)) { throw new Exception("Undefined packet received"); }
    ...
    public T ReadKnownPacket<T>(NetDataReader reader) where T : class, new()
    {
        ...
        if (typeHash != info.Hash) { return null; }
    ...
    public bool ReadKnownPacket<T>(NetDataReader reader, T target) where T : class, new()
    {
        var info = ReadInfo(reader);
        ulong typeHash = _hasher.GetHash(typeof(T).Name);
        if (typeHash != info.Hash) { return false; }
    ...
    {
        var info = ReadInfo(reader);
        if (info.CreatorFunc != null) { info.Reference = info.CreatorFunc(); }
    ...
    {
        var info = RegisterInternal<T>();
        if (packetConstructor != null) { info.CreatorFunc = () => packetConstructor(); }
