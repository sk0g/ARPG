...
public sealed class NetManager
{
    ...
    {
        get { return _logicThread.SleepTime; }
    ...
    public void AddFlowMode(int startRtt, int packetsPerSecond)
    {
        ...
        if (_flowModes.Count > 0 && startRtt < _flowModes[0].StartRtt) { _flowModes.Insert(0, fm); }
        ...
        int count)
    {
        if (sendDisconnectPacket)
        {
            ...
            if (data != null) { Buffer.BlockCopy(data, start, disconnectPacket.RawData, 9, count); }
    ...
    private NetEvent CreateEvent(NetEventType type)
    {
        ...
        lock (_netEventsPool)
        {
            if (_netEventsPool.Count > 0) { evt = _netEventsPool.Pop(); }
        }

        if (evt == null) { evt = new NetEvent(); }
    ...
    {
        if (UnsyncedEvents) { ProcessEvent(evt); }
        else
        {
            lock (_netEventsQueue) { _netEventsQueue.Enqueue(evt); }
        }
    }
    ...
    private void ProcessEvent(NetEvent evt)
    {
        ...
        case NetEventType.DiscoveryResponse:
            _netEventListener.OnNetworkReceiveUnconnected(evt.RemoteEndPoint, evt.DataReader,
                                                          UnconnectedMessageType.DiscoveryResponse);
            break;
        ...
        lock (_netEventsPool) { _netEventsPool.Push(evt); }
    }
    ...
    private void UpdateLogic()
    {
        ...
        lock (_peers)
        {
            ...
            for (int i = 0; i < _peers.Count; i++)
            {
                ...
                {
                    NetUtils.DebugWrite("[NM] Disconnect by timeout: {0} > {1}", netPeer.TimeSinceLastPacket,
                                        DisconnectTimeout);
                    var netEvent = CreateEvent(NetEventType.Disconnect);
    ...
    private void DataReceived(byte[] reusableBuffer, int count, NetEndPoint remoteEndPoint)
    {
        ...
        if (_peers.TryGetValue(remoteEndPoint, out netPeer))
        {
            ...
            else { netPeer.ProcessPacket(packet); }
    ...
    public void SendToAll(byte[] data, int start, int length, SendOptions options)
    {
        lock (_peers)
        {
            for (int i = 0; i < _peers.Count; i++) { _peers[i].Send(data, start, length, options); }
        }
    }
    ...
    public void SendToAll(byte[] data, int start, int length, SendOptions options, NetPeer excludePeer)
    {
        lock (_peers)
        {
            ...
            {
                var netPeer = _peers[i];
                if (netPeer != excludePeer) { netPeer.Send(data, start, length, options); }
            }
        }
    }
    ...
    public void Flush()
    {
        lock (_peers)
        {
            for (int i = 0; i < _peers.Count; i++) { _peers[i].Flush(); }
        }
    }
    ...
    public void PollEvents()
    {
        ...
        {
            NetEvent evt;
            lock (_netEventsQueue) { evt = _netEventsQueue.Dequeue(); }
    ...
    {
        if (!IsRunning) { throw new Exception("Client is not running"); }
    ...
    public NetPeer GetFirstPeer()
    {
        lock (_peers)
        {
            if (_peers.Count > 0) { return _peers[0]; }
        }
    ...
    public void GetPeersNonAlloc(List<NetPeer> peers)
    {
        ...
        lock (_peers)
        {
            for (int i = 0; i < _peers.Count; i++) { peers.Add(_peers[i]); }
        }
    }
